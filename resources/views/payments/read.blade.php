...
@section('content')
    ...
    $orderby = strtolower($_GET['orderBy'] ?? 'desc');
    $orderby = $orderby === 'asc' ? 'asc' : 'desc';
    $sortableFields = [
        'id' => 'payments.id',
        'payment_date' => 'payments.payment_date',
        'amount' => 'payments.amount',
        'kumon_month' => 'payments.kumon_month',
        'payment_type' => 'payments.payment_type',
    ];
    $fieldKey = $_GET['field'] ?? 'payment_date';
    $field = $sortableFields[$fieldKey] ?? 'payments.payment_date';

    $search = trim(request('search', ''));
    $clientFilter = request('client_id');
    $paymentTypeFilter = request('payment_type');
    $kumonMonthFilter = request('kumon_month');

    $qry = DB::table('client_payments as payments')->where('payments.is_deleted', 0)
        ->leftJoin('clients as clients', 'payments.client_id', '=', 'clients.id')
        ->select(
            'payments.*',
            'clients.client_display_name',
            'clients.firstname',
            'clients.lastname',
            'clients.company_name',
            DB::raw("TRIM(COALESCE(NULLIF(clients.client_display_name, ''), NULLIF(clients.company_name, ''), CONCAT(COALESCE(clients.firstname, ''), ' ', COALESCE(clients.lastname, '')))) as client_name")
        );

    if (!(Auth::user()->role == 'admin' || Auth::user()->role == 'staff')) {
        $qry->where('payments.client_id', Auth::user()->id);
    }

    if (!empty($clientFilter)) {
        $qry->where('payments.client_id', $clientFilter);
    }

    if (!empty($paymentTypeFilter)) {
        $qry->where('payments.payment_type', $paymentTypeFilter);
    }

    if (!empty($kumonMonthFilter)) {
        $qry->where('payments.kumon_month', 'like', '%' . $kumonMonthFilter . '%');
    }

    if ($search !== '') {
        $qry->where(function ($query) use ($search) {
            $query
                ->orWhere('clients.client_display_name', 'like', '%' . $search . '%')
                ->orWhere('clients.company_name', 'like', '%' . $search . '%')
                ->orWhere(DB::raw("TRIM(CONCAT(COALESCE(clients.firstname, ''), ' ', COALESCE(clients.lastname, '')))"), 'like', '%' . $search . '%')
                ->orWhere('payments.payment_type', 'like', '%' . $search . '%')
                ->orWhere('payments.kumon_month', 'like', '%' . $search . '%')
    ...
    $totalRows = $totalQuery->select(DB::raw('COUNT(DISTINCT payments.id) as aggregate'))->first()->aggregate;
                    ...
                    <div class="TopArea" style="position: sticky;padding-top: 12px;z-index: 1000;padding-bottom: 7px;max-height: 62px;">
                        <div class="row align-items-center">
                            ...
                            $filter =
                                ...
                                (isset($_GET['payment_type']) ? '&payment_type=' . $_GET['payment_type'] : '') .
                                (isset($_GET['kumon_month']) ? '&kumon_month=' . $_GET['kumon_month'] : '') .
                                            ...
                                            <form class="push mb-0" method="get" id="form-search"
                                                ...
                                                <div class="input-group mainser banner-icon">
                                                    <input type="text" value="{{ @$_GET['search'] }}"
                                                        ...
                                                        placeholder="Search Payments" style="font-weight: 300!important;">
                                    ...
                                    <div class="col-lg-6 d-flex align-items-center justify-content-end">
                                        ...
                                        <div class="d-flex text-right  w-  justify-content-end">
                                            <form id="limit_form" class="ml-2 mb-0"
                                                action="{{ url('payments') }}?{{ $_SERVER['QUERY_STRING'] }}">
                        ...
                        <div class="row">
                            <div class="col-md-4 bubble-header">
                                <div class="row align-items-center">
                                    ...
                                    <p class="col-11 bubble-header-text d-flex justify-content-between align-items-center">Payments
                                        <span class="{{ $get_text_display }} text-right" style="line-height: 1.3;">
                                            <a class="clear-link" href="{{ url('/payments') }}"
                                    ...
                                    <div class="d-flex align-items-center w-100  " style="justify-content:space-between;">
                                        ...
                                        <div class="d-flex justify-content-center align-items-center flex-grow-1"
                                            ...
                                            <div class="ms-3 flex-grow-1" style="min-width: 0; margin-left: 0.91rem;">
                                                <div class="d-flex align-items-center">
                                                    <div class="header-badge flex-shrink-0">PAY</div>
                                                    <h4
                                            ...
                                            @if ($totalRows != 0)
                                                {{-- <span data-toggle="modal" data-bs-target="#AttachmentModal"
                                                    ...
                                                    <a href="javascript:;" d=""
                                                        ...
                                                        <i class="fa-light fa-paperclip-vertical regular-icon"></i>
                                                        <i class="fa-solid fa-paperclip-vertical solid-icon"
                                                ...
                                                <span data-toggle="modal" data-bs-target="#CommentModal" data-target="#CommentModal">
                                                    <a href="javascript:;" d=""
                                                        ...
                                                        <i class="fa-light fa-comment-dots regular-icon"></i>
                                                        <i class="fa-solid fa-comment-dots solid-icon"
                                                ...
                                                {{-- <a href="javascript:;"
                                                    ...
                                                    <i class="fa-light fa-circle-envelope regular-icon"></i>
                                                ...
                                                <a href="javascript:;"
                                                    ...
                                                    <i class="fa-light fa-message-exclamation regular-icon"></i>
                                                    <i class="fa-solid fa-message-exclamation solid-icon"
                                                        style="padding-right: 2px; padding-left: 2.5px;"></i>
                                                </a> --}}
                                                ...
                                                {{-- <a href="javascript:;" class="text-white edit-icon btn-edit banner-icon sub-nav-icons"
                                                    ...
                                                    <i class="fa-light fa-file-pen regular-icon"></i>
                                                    <i class="fa-solid fa-file-pen solid-icon"></i>
                                                </a> --}}
                                                <a href="javascript:;" d=""
                        ...
                        <div class="row">
                            <div class="col-md-4 bubble-header">
                                ...
                                <div class="filter-dropdown-container roll-down">
                                    <div class="row align-items-center">
                                        <div class="col-1 bubble-fil ter-ht {{ $class }}" style="height: 284px;"></div>
                                        <form id="filterForm" method="GET" action=""
                                            ...
                                            <div class="block block-transparent mb-0">
                                                <div class="pl-3 pt-0 pb-0">
                                                    ...
                                                    <div class="align-items-baseline row">
                                                        ...
                                                        <div class="col-sm-7 px-sm-0 form-group">
                                                                <select class="form-control selectpicker"
                                                                    ...
                                                                    @foreach ($client as $c)
                                                                        <option value="{{ $c->id }}"
                                                                            {{ @$_GET['client_id'] == $c->id ? 'selected' : '' }}>{{ $c->firstname . ' ' . $c->lastname }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                    </div>
                                                    <div class="align-items-baseline row">
                                                        <label class="col-sm-4 modal-label">Payment Type</label>
                                                        <div class="col-sm-7 px-sm-0 form-group">
                                                            <select id="payment_type" name="payment_type"
                                                                ...
                                                                @foreach (['E-Transfer', 'Cash', 'Credit Card', 'EFT'] as $type)
                                                                    <option value="{{ $type }}"
                                                    ...
                                                    <div class="align-items-baseline row">
                                                        <label class="col-sm-4 modal-label">Kumon Month</label>
                                                        <div class="col-sm-7 px-sm-0 form-group">
                                                            <input type="text" id="kumon_month" name="kumon_month"
                                                                class="form-control"
                                                                value="{{ request('kumon_month') }}"
                                                ...
                                                <div class="block-content text-right pt-1 pr-0" style="padding-left: 9mm;">
                                                    <a href="{{ url('/payments') }}" class="btn btn-action mr-3">Clear</a>
                                    ...
                                    <div style="padding-top: 15px;">
                                        @foreach ($qry as $q)
                                            <div class="block block-rounded   table-block-new mb-2 pb-0  -  viewContent"
                                                data="{{ $q->id }}"
                                                data-client-name="{{ $q->client_name ?: 'Client #' . $q->client_id }}"
                                                data-payment-summary="{{ $q->kumon_month }} - {{ $q->payment_type }}"
                                                ...
                                                <div class="block-content p-2 d-flex">
                                                    ...
                                                    <div class="align-content-center" style="width:82%;">
                                                        <div class="d-flex align-items-center justify-content-between">
                                                            ...
                                                            <div class="d-flex align-items-center">
                                                                <span class="c4 font-9pt">${{ number_format((float) $q->amount, 2) }}</span>
                                                            </div>
                                                        </div>
                                                        <div class="w-100">
                                                            <div class="d-flex align-items-center mb-0">
                                                                <i class="fa-thin fa-calendar-days fs-18 mr-1 mb-0"></i>
                                                                <p class="font-12pt mb-0 text-truncate titillium-web-light ml-1">
                                                                    {{ \Carbon\Carbon::parse($q->payment_date)->format('d-M-Y') }}
                                                                </p>
                                                            </div>
                                                        </div>
                                        ...
                                        <div class="col-md-12 bg-nav-tab position-relative">
                                            <nav class="align-items-center d-flex justify-content-between read-nav-tabs sub-nav-tabs">
                                                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                                    <button class="nav-link border-right-0 active" id="nav-main-tab-vacation"
                                        ...
                                        <div class="block  block-transparent mb-0">
                                            ...
                                            <div class="block-content block-content-full   pt-4"
                                                ...
                                                @if (isset($_GET['advance_search']))
                                                    <a href="{{ url('payments') }}" class="btn     btn-new-secondary float-right"
                    ...
                    function showData(id) {
                        ...
                        $.ajax({
                            ...
                            success: function(res) {
                                ...
                                $('#showCards').addClass('d-none').empty();
                                const selectedItem = $('.viewContent[data=' + id + ']');
                                const clientName = selectedItem.data('client-name');
                                const paymentSummary = selectedItem.data('payment-summary');
                                $('.header-item-code').text(clientName || 'Payment');
                                $('.header-desc').text(paymentSummary || '');
                                // $('.header-image').attr('src', res.header_img);
                                // $('.btn-edit').attr('data', res.id);
                                // $('.btnDelete').attr('data', res.id);
                                // $('.btn-clone').attr('data', res.id);
                                $('.sub-nav-icons').attr('data', id);
                                $('.sub-nav-icons').attr('data-id', id);
                                $('.sub-nav-icons').attr('data-item-id', id);
                                $('.closeEditBtn').attr('data-item-id', id);
                                $('.closeEditBtn').attr('data', id);
