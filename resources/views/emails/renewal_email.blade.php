...
<?php
   ...
   $hash=uniqid().Hash::make(time().rand(1,100000000000000));
                ...
                ?>
                   {{-- <h4>Description :  {{$data['contract_description']}}</h4> --}}
                   <h4>{{ $data['contract_description'] }}</h4>
  
                  <h4>Vendor :  {{$data['vendor_name']}}</h4>
 
 <h4>Contract # : {{$data['contract_no']}}  </h4>
  <h4>Start Date :  {{date('Y/M/d',strtotime($data['start_date']))}}</h4>
