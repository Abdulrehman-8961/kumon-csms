...
$html.='  <div class="block new-block position-relative mt-5" >
                                                <div class="top-div text-capitalize">Contract Assets</div>
                                            ...
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td class="text-right" style="width: 130px;">
                                                            ...
                                                            <h3 class="mb-0">$'.number_format($c->msrp,2).'</h3>
                                                    </td>
                                                </tr>
                                                <tr >
                                                    <td colspan="2"  class="pt-0" style="vertical-align: top;" >
                                                       <div class="pl-2  mb-0 row  "> ';
                                                                     $line_item=DB::Table('contract_assets as ca')->select('a.hostname','a.AssetStatus','a.id','at.asset_type_description','a.sn','a.asset_type')->where('ca.contract_id',$q->id)->where('ca.contract_detail_id',$c->contract_detail_id)->join('assets as a','a.id','=','ca.hostname')->leftjoin('asset_type as at','a.asset_type_id','=','at.asset_type_id')->groupBy('ca.hostname')->where('ca.is_deleted',0)->orderBy('a.hostname','asc')->get();

 
                                                                    foreach($line_item as $l){
                                                                                if($l->AssetStatus!=1){
                                                                                      $html.='  <div class="col-lg-2   px-2"><a href="print-asset?id='.$l->id.'" target="_blank"><div data-toggle="tooltip" data-html="true" data-original-title="'.$l->asset_type_description.($l->asset_type=='physical'?'<br>'.$l->sn:'').'" title="" class="w-100   AssetInactive">'.$l->hostname.'</div></a></div>';
                                                                                }else{
                                                                                    $html.='<div class="col-lg-2   px-2"><a href="print-asset?id='.$l->id.'" target="_blank"><div data-toggle="tooltip" data-html="true" data-original-title="'.$l->asset_type_description.($l->asset_type=='physical'?'<br>'.$l->sn:'').'" title="" class="w-100   AssetActive">'.$l->hostname.'</div></a></div>';
                                                                                }
                                                                                }
