          ...
          <thead class="thead thead-dark">
              ...
              <tr>
                  ...
                  @if (in_array(24, $column_array))
                      <td style="min-width:100px;width:100px;"><strong>DL Type</strong></td>
                  @endif
              </tr>
              ...
              <tr>
                  ...
                  @if (in_array(25, $column_array))
                      <th data-index=1 style="min-width: 130px">Managed By </th>
                  @endif
                  ...
                  @if (in_array(26, $column_array))
                      <th data-index=6 style="min-width: 100px">Currency </th>
                  @endif
                  @if (in_array(27, $column_array))
                      <th data-index=6 style="min-width: 100px">Total Amount </th>
                  @endif
          ...
          <tbody id="showdata">
              ...
              @foreach ($qry as $q)
                  <tr data="{{ $q->id }}">
                      ...
                      @if (in_array(2, $column_array))
                          <td data-index=2>{{ $q->client_display_name }}</td>
                      @endif
                      @if (in_array(25, $column_array))
                          <td data-index=2>{{ $q->managed_by }}</td>
                      @endif
                      @if (in_array(3, $column_array))
                      ...
                      @endif
                      @if (in_array(26, $column_array))
                          <td data-index=7>{{ $q->currency }}</td>
                      @endif
                      @if (in_array(27, $column_array))
                          <td data-index=7>{{ $q->total_amount }}</td>
                      @endif
                      @if (in_array(8, $column_array))
                  ...
                  <?php $details = DB::Table('contract_details')
                      ->where('contract_id', $q->id)
                      ->where('is_deleted', 0)
                      ->get();
                  $count = 0;
                  ...
                  ?>
                  @if (in_array(19, $column_array) ||
                          in_array(20, $column_array) ||
                          in_array(21, $column_array) ||
                          in_array(22, $column_array))
                      @foreach ($details as $key => $d)
                          <?php $count++;
                          $asset = DB::Table('contract_assets as ca')
                              ->selectRaw('Group_Concat(a.hostname) as asset_name')
                              ->leftjoin('assets as  a', 'a.id', '=', 'ca.hostname')
                              ->where('ca.contract_detail_id', $d->contract_detail_id)
                          ...
                          <tr>
                              ...
                              @endif
                              @if (in_array(24, $column_array))
                                  <td data="24">{{ $d->contract_type_line }} </td>
                              @endif
