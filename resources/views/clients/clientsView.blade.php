...
@section('content')
    ...
    <style>
        ...
        .field-action-wrapper:hover .field-action-btn {
            opacity: 1;
            pointer-events: auto;
        }
    ...
    if (sizeof($_GET) > 0) {
        ...
        if (isset($_GET['advance_search'])) {
            ...
            if (isset($_GET['client_status']) && $_GET['client_status'] != '') {
                $contract_type = $_GET['client_status'];
                $cond .= " and a.client_status like '%$contract_type%'";
            }
        }
        ...
        if (@Auth::user()->role == 'admin') {
            $qry = DB::table('clients')
                ->where(function ($query) {
                    ...
                    $query->Orwhere('city', 'like', '%' . @$_GET['search'] . '%');
                    $query->Orwhere('zip', 'like', '%' . @$_GET['search'] . '%');
                    $query->Orwhere('state', 'like', '%' . @$_GET['search'] . '%');
                    $query->Orwhere('country', 'like', '%' . @$_GET['search'] . '%');
                    $query->Orwhere('payment_method', 'like', '%' . @$_GET['search'] . '%');
                    $query->Orwhere('website', 'like', '%' . @$_GET['search'] . '%');
                })
                ->when($clientStatus !== null && $clientStatus !== '', function ($query) use ($clientStatus) {
                    $query->where('is_active', $clientStatus);
                })
            ...
            $qry = DB::table('clients')
                ...
                ->when($clientStatus !== null && $clientStatus !== '', function ($query) use ($clientStatus) {
                    $query->where('client_status', $clientStatus);
                })
                ->when($clientId !== null && $clientId !== '', function ($query) use ($clientId) {
                    $query->where('id', $clientId);
                })
        ...
        $qry = DB::table('clients')->where('is_deleted', 0)->orderBy('id', 'desc');
        $totalQuery = clone $qry;
        $totalRows = $totalQuery->select(DB::raw('COUNT(DISTINCT id) as aggregate'))->first()->aggregate;
        $qry = $qry->orderBy($field, $orderby)->paginate($limit);
    }
    ...
    <main id="main-container pt-0">
        ...
        <style type="text/css">
            ...
            .student-table tbody tr:hover span {
                font-weight: 700 !important;
                color: #0665d0 !important;
            }

            .payment-table tbody tr:hover td,
            .payment-table tbody tr:hover span,
            .vacation-table tbody tr:hover td,
            .vacation-table tbody tr:hover span {
                font-weight: 600 !important;
                color: #0665d0 !important;
            }
            ...
            .added-pinned-message .drag-handle {
                opacity: 0;
                pointer-events: none;
                transition: opacity 0.15s ease-in-out;
            }

            .added-pinned-message .pinned-message-item:hover .drag-handle {
                opacity: 1;
                pointer-events: auto;
            }
            ...
            /* each radio wrapper */
            .managed-radio-group .contract_type_button {
                flex: 1;
                display: flex;
            }
            ...
            /* Left side should shrink properly inside flex */
            .msg-left {
                flex: 1 1 auto;
                min-width: 0;
            ...
            /* Reserved area for user/time on the right */
            .msg-meta {
                flex: 0 0 220px;
            ...
            /* optional charcoal helper if you don't have it */
            .text-charcoal {
                color: #3f3f3f;
            }
            ...
            .confirm-method-change:hover {
                background: #FFCC62 !important;
            }
            ...
            .tax-receipt-students-table {
                width: 100%;
                border-collapse: separate;
                border-spacing: 0 0px;
            }

            .tax-receipt-students-table tbody td {
                padding: 4px 6px;
                vertical-align: middle;
            }
            ...
            .tax-receipt-students-table tbody tr:hover td {
                border-top-color: #ccccccad !important;
                border-bottom-color: #ccccccad !important;
            }

            .tax-receipt-students-table tbody td:first-child {
                border-left: 1px solid transparent;
            }

            .tax-receipt-students-table tbody td:last-child {
                border-right: 1px solid transparent;
            }

            .tax-receipt-students-table tbody tr:hover td:first-child {
                border-left-color: #ccccccad !important;
                border-top-left-radius: 10px;
                border-bottom-left-radius: 10px;
            }

            .tax-receipt-students-table tbody tr:hover td:last-child {
                border-right-color: #ccccccad !important;
            ...
            .tax-receipt-students-table .form-control {
                height: 30px;
                font-size: 13px;
                border-radius: 8px;
            }

            .tax-receipt-students-table .tax-receipt-remove {
                opacity: 0;
                pointer-events: none;
                transition: opacity 0.15s ease-in-out;
            }

            .tax-receipt-students-table tbody tr:hover .tax-receipt-remove,
            .tax-receipt-students-table tbody tr:focus-within .tax-receipt-remove {
                opacity: 1;
                pointer-events: auto;
            }
            .tax-receipt-students-table .tax-receipt-edit {
                opacity: 0;
                pointer-events: none;
                transition: opacity 0.15s ease-in-out;
            }

            .tax-receipt-students-table .tax-receipt-amount-head,
            .tax-receipt-students-table .tax-receipt-amount-cell {
                padding-right: 18px !important;
            }

            .tax-receipt-students-table tbody tr:hover .tax-receipt-edit,
            .tax-receipt-students-table tbody tr:focus-within .tax-receipt-edit {
                opacity: 1;
                pointer-events: auto;
            }
        ...
        <div class="con no-print page-header py-1" id="">
            ...
            <div class="b   mb-0  ">
                <div class="block-content pt-0 mt-0">
                    <div class="TopArea"
                        style="position: sticky;padding-top: 12px;z-index: 1000;padding-bottom: 7px;max-height: 62px;">
                        <div class="row align-items-center">
                            <?php
                            $filter = (isset($_GET['advance_search']) ? 'advance_search=' . $_GET['advance_search'] : '') . (isset($_GET['client_id']) ? '&client_id=' . $_GET['client_id'] : '') . (isset($_GET['client_status']) ? '&client_status=' . $_GET['client_status'] : '') . (isset($_GET['id']) ? '&id=' . $_GET['id'] : '');
                            $filter_2 = (isset($_GET['advance_search']) ? 'advance_search=' . $_GET['advance_search'] : '') . (isset($_GET['client_id']) ? '&client_id=' . $_GET['client_id'] : '') . (isset($_GET['client_status']) ? '&client_status=' . $_GET['client_status'] : '') . (isset($_GET['limit']) ? '&limit=' . $_GET['limit'] : '');
                            ?>
                            <div class="col-sm-4">
                                @Auth
                                    <div class="row align-items-center">
                                            ...
                                            <form class="push mb-0" method="get" id="form-search"
                                                ...
                                                <div class="input-group mainser banner-icon">
                                                    <input type="text" value="{{ @$_GET['search'] }}"
                                                        ...
                                                        placeholder="Search Clients" style="font-weight: 300!important;">
                                ...
                                <div class="student-download-toast client-export-toast" role="status" aria-live="polite">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <div class="d-flex align-items-center">
                                            <i class="fa-light fa-circle-check mr-2"></i>
                                            <span class="font-titillium fs-14 text-darkgrey">Export completed
                                                successfully!</span>
                                        </div>
                                        <button type="button" data-section="client-export-toast"
                                            class="btn p-0 btn-close-toast text-darkgrey" aria-label="Close">
                                            <i class="fa-light fa-xmark"></i>
                                        </button>
                                    </div>
                                </div>
                                <div class="student-download-toast client-import-toast" role="status" aria-live="polite">
                                    <div class="d-flex align-items-center justify-content-between">
                                        <div class="d-flex align-items-center">
                                            <i class="fa-light fa-circle-check mr-2"></i>
                                            <span class="font-titillium fs-14 text-darkgrey">Import completed
                                                successfully!</span>
                                        </div>
                                        <button type="button" data-section="client-import-toast"
                                            class="btn p-0 btn-close-toast text-darkgrey" aria-label="Close">
                                            <i class="fa-light fa-xmark"></i>
                                        </button>
                                    </div>
                                ...
                                <div class="col-sm-2" style="margin-top: 2px;">
                                    @Auth
                                        <a class="btn btn-dual d2 banner-icon p-2 btn-add" data-toggle="tooltip"
                                            data-trigger="hover" data-placement="top" title=""
                                            data-original-title="Add Client" href="javascript:;">
                                        ...
                                        <span>
                                            <button class="btn btn-dual d2 banner-icon p-2" data-toggle="tooltip"
                                                data-trigger="hover" data-placement="top" title=""
                                                data-original-title="Export" href="javascript:;" id="openExportModal">
                                                <i class="fa-light fa-arrow-up-from-bracket text-white fs-22"></i>
                                            </button>
                                        </span>
                                        <span>
                                            <button class="btn btn-dual d2 banner-icon p-2" data-toggle="tooltip"
                                                data-trigger="hover" data-placement="top" title=""
                                                data-original-title="Import" href="javascript:;" id="openImportModal">
                                                <i class="fa-light fa-arrow-down-from-bracket text-white fs-22"></i>
                                            </button>
                                        </span>
                                    ...
                                    <div class="col-lg-6 d-flex align-items-center justify-content-end">
                                        ...
                                        <div class="d-flex text-right  w-  justify-content-end">
                                            ...
                                            <a href="javascript:;" id="centerSettingsBtn" data-toggle="tooltip"
                                                data-trigger="hover" data-placement="top" title=""
                                                data-original-title="Center Settings"
                                                class="mr-2 text-dark d3 banner-icon p-2 btn btn-dual align-content-center">
                        ...
                        <div class="row">
                            <div class="col-md-4 bubble-header">
                                <div class="row align-items-center">
                                    ...
                                    <p class="col-11 bubble-header-text d-flex justify-content-between align-items-center">Clients
                                        <span class="{{ $get_text_display }} text-right" style="line-height: 1.3;">
                                            <a class="clear-link" href="{{ url('/clients') }}" data-custom-class="header-tooltip"
                                                data-toggle="tooltip" data-trigger="hover" data-placement="top" title=""
                                                data-original-title="Clear" style="color: #595959;">
                                    ...
                                    <div class="d-flex align-items-center w-100  " style="justify-content:space-between;">
                                        ...
                                        <div class="d-flex justify-content-center align-items-center flex-grow-1"
                                            ...
                                            <div class="ms-3 flex-grow-1" style="min-width: 0; margin-left: 0.91rem;">
                                                <div class="d-flex align-items-center">
                                                    <div class="header-badge flex-shrink-0">KUM</div>
                                                    <h4
                                        ...
                                        <div class="new-header-icon-div d-flex align-items-center flex-shrink-0 no-print">
                                            @if ($totalRows != 0)
                                                <a href="javascript:;"
                                                    class="text-white email-icon btn-pin-message banner-icon sub-nav-icons"
                                                    data-item-id="{{ @$GETID }}" data-id="{{ @$GETID }}"
                                                    data-toggle="tooltip" data-trigger="hover" data-placement="top" title=""
                                                    data-original-title="Pin Message">
                                                    <i class="fa-light fa-message-exclamation fs-20 regular-icon"></i>
                                                    <i class="fa-solid fa-message-exclamation fs-20 solid-icon"
                                                        style="padding-right: 2px; padding-left: 2.5px;"></i>
                                                </a>
                                                <span data-toggle="modal" data-bs-target="#AttachmentModal"
                                                    data-target="#AttachmentModal">
                                                    <a href="javascript:;" d=""
                                                        class="text-white attachment-icon banner-icon sub-nav-icons"
                                                        data-item-id="{{ @$GETID }}" data-item-code="" data-toggle="tooltip"
                                                        data-trigger="hover" data-placement="top" title=""
                                                        data-original-title="Add Attachment">

                                                        <i class="fa-light fa-paperclip-vertical fs-20 regular-icon"></i>
                                                        <i class="fa-solid fa-paperclip-vertical fs-20 solid-icon"
                                                            style="padding-right: 3px; padding-left: 4.5px;"></i>
                                                    </a></span>
                                                <span data-toggle="modal" data-bs-target="#CommentModal" data-target="#CommentModal">
                                                    <a href="javascript:;" d=""
                                                        class="text-white comment-icon banner-icon sub-nav-icons"
                                                        data-item-id="{{ @$GETID }}" data-item-code="" data-toggle="tooltip"
                                                        data-trigger="hover" data-placement="top" title=""
                                                        data-original-title="Add Comment">
                                                        <i class="fa-light fa-message-plus fs-20 regular-icon"></i>
                                                        <i class="fa-solid fa-message-plus fs-20 solid-icon"
                                                            style="padding-right: 2px; padding-left: 3px;"></i>
                                                    </a>
                                                </span>
                                                ...
                                                <a data-toggle="tooltip" data-trigger="hover" data-placement="top" title=""
                                                    ...
                                                    <i class="fa-solid fa-clone fs-20 solid-icon"
                                                        style="padding-right: 2px; padding-left: 3px;"></i>
                                                    <!-- <img src="{{ asset('public/icons/icon-white-clone.png') }}" width="22px"> -->
                                                </a>
                                                <a href="javascript:;" type="button" data-toggle="tooltip" data-trigger="hover"
                                                    ...
                                                    <i class="fa-light fa-circle-share-nodes regular-icon"
                                                        style="font-size: 21px;"></i>
                                                    <i class="fa-solid fa-circle-share-nodes solid-icon"
                                                        style="padding-right: 2px; padding-left: 3px;font-size: 21px;"></i>
                                                </a>


                                                <a href="javascript:;"
                                                    class="text-white email-icon btn-email banner-icon sub-nav-icons"
                                                    data-item-id="{{ @$GETID }}" data-id="{{ @$GETID }}"
                                                    data-toggle="tooltip" data-trigger="hover" data-placement="top" title=""
                                                    data-original-title="Email">
                                                    <i class="fa-light fa-circle-envelope regular-icon" style="font-size: 21px;"></i>
                                                    <i class="fa-solid fa-circle-envelope solid-icon"
                                                        style="padding-right: 2px; padding-left: 3px;font-size: 21px;"></i>
                                                </a>

                                                <a href="javascript:;" d="" class="text-white tax-reciept-btn banner-icon sub-nav-icons"
                                                    data-item-id="{{ @$GETID }}" data-item-code="" data-toggle="tooltip"
                                                    data-trigger="hover" data-placement="top" title=""
                                                    data-original-title="Tax Receipt">
                                                    <i class="fa-light fa-file-invoice-dollar fs-20 regular-icon"></i>
                                                    <i class="fa-solid fa-file-invoice-dollar fs-20 solid-icon"
                                                        style="padding-right: 5px;padding-left: 5px;"></i>
                                                ...
                                                <a href="javascript:;" d=""
                                                    ...
                                                    <!-- <img src="{{ asset('public/img/cf-menu-icons/header-delete.png') }}" width="24px"> -->
                                                </a>
                                ...
                                <div class="h-100 d-flex align-items-center">
                                    <div class="d-flex align-items-center w-100  " style="justify-content:space-between;">
                                        ...
                                        <div class="new-header-icon-div d-flex align-items-center  no-print">
                                            <a href="javascript:;" d="" class="text-white attachment-icon banner-icon"
                                                data-item-id="{{ @$GETID }}" data-item-code="" data-toggle="tooltip"
                                                data-trigger="hover" data-placement="top" title=""
                                                data-original-title="Add Attachment">
                                                <span data-toggle="modal" data-bs-target="#AttachmentModalAdd"
                                                    data-target="#AttachmentModalAdd">
                                                    <i class="fa-light fa-paperclip-vertical fs-23 regular-icon"></i>
                                                    <i class="fa-solid fa-paperclip-vertical fs-23 solid-icon"
                                                        style="padding-right: 3px; padding-left: 2px;"></i>
                                                </span>
                                            </a>
                                            <a href="javascript:;" d="" class="text-white comment-icon banner-icon"
                                                data-item-id="{{ @$GETID }}" data-item-code="" data-toggle="tooltip"
                                                data-trigger="hover" data-placement="top" title=""
                                                data-original-title="Add Comment">
                                                <span data-toggle="modal" data-bs-target="#CommentModalAdd"
                                                    data-target="#CommentModalAdd">
                                                    <i class="fa-light fa-message-plus fs-23 regular-icon"></i>
                                                    <i class="fa-solid fa-message-plus fs-23 solid-icon"
                        ...
                        <div class="row">
                            <div class="col-md-4 bubble-header">
                                ...
                                <div class="filter-dropdown-container roll-down">
                                    <div class="row align-items-center">
                                        <div class="col-1 bubble-fil ter-ht {{ $class }}" style="height: 244px;"></div>
                                        <form id="filterForm" method="GET" action="{{ url('/clients') }}"
                                            ...
                                            <div class="block block-transparent mb-0">
                                                <div class="pl-3 pt-0 pb-0">
                                                    <div class="align-items-baseline row">
                                                        <label class="col-sm-4 modal-label">Client Status</label>
                                                        ...
                                                        <div class="col-sm-7 px-sm-0 form-group">
                                                            <div class="p-2 mb-2 position-relative"
                                                                ...
                                                                <div class="d-flex align-items-center pb-1 pl-1 pt-0">
                                                                    {{-- <i class="fa-light fa-certificate text-grey fs-18 constant-icon mr-2"></i> --}}

                                                                    <div class="managed-radio-group d-flex gap-2 flex-wrap">

                                                                        <div class="contract_type_button">
                                                                            <input type="radio" id="client_status_all"
                                                                                name="client_status" value=""
                                                                                {{ !request()->has('client_status') || request('client_status') === '' ? 'checked' : '' }}>
                                                                            <label class="btn radio-label fw-300 py-1 fs-14 w-100"
                                                                                for="client_status_all">
                                                                                All
                                                                            </label>
                                                                        </div>

                                                                        <div class="contract_type_button">
                                                                            <input type="radio" id="client_status_active"
                                                                                name="client_status" value="1"
                                                                                {{ @$_GET['client_status'] === 'Active' ? 'checked' : '' }}>
                                                                            <label class="btn radio-label fw-300 py-1 fs-14 w-100"
                                                                                for="client_status_active">
                                                                                Active
                                                                            </label>
                                                                        </div>

                                                                        <div class="contract_type_button">
                                                                            <input type="radio" id="client_status_inActive"
                                                                                name="client_status" value="0"
                                                                                {{ @$_GET['client_status'] === 'inActive' ? 'checked' : '' }}>
                                                                            <label class="btn radio-label fw-300 py-1 fs-14 w-100"
                                                                                for="client_status_inActive">
                                                                                inActive
                                                                            </label>
                                                                        </div>
                                                ...
                                                <div class="block-content text-right pt-1 pr-0" style="padding-left: 9mm;">
                                                    <a href="{{ url('/clients') }}" class="btn btn-action mr-3">Clear</a>
                                                    ...
                                                    <div class="align-content-center" style="width:82%;">
                                                        <div class="d-flex align-items-center justify-content-between">
                                                            ...
                                                            <div>
                                                                <div>
                                                                    @if ($q->is_active)
                                                                        <i class="fa-light fa-circle-check text-darkgreen fs-20"
                                                                            data-toggle="tooltip" data-trigger="hover"
                                                                            data-placement="top" data-title="" data-html="true"
                                                                            data-original-title="Active"></i>
                                                                    @else
                                                                        <i class="fa-light fa-octagon-exclamation text-red fs-20"
                                                                            data-toggle="tooltip" data-trigger="hover"
                                                                            data-placement="top" data-title="" data-html="true"
                                                                            data-original-title="Ended"></i>
                                                                    @endif
                                                                </div>
                                                            </div>
                                                        </div>
                                                        <div class="w-100">
                                                            <div class="d-flex align-items-center mb-0">
                                                                @if (@$q->payment_method == 'EFT')
                                                                    <i class="fa-thin fa-money-bill-transfer fs-18 mr-1 mb-0"></i>
                                                                @elseif(@$q->payment_method == 'Cash')
                                                                    <i class="fa-thin fa-money-bill fs-18 mr-1 mb-0"></i>
                                                                @else
                                                                    <i class="fa-thin fa-credit-card fs-18 mr-1 mb-0"></i>
                                                                @endif
                                                            ...
                                                            @php
                                                                $statusIcon = function ($subject) {
                                                                    return match (strtolower($subject)) {
                                                                        'active' => 'fa-light fa-circle-check text-green',
                                                                        default => 'fa-light fa-circle-xmark text-red',
                                                                    };
                                                                };
                                                                $paymentIcon = function ($payment) {
                                                                    return match (strtolower($payment)) {
                                                                        'E-Transfer'
                                                                            => 'fa-light fa-money-billtransfer',
                                                                        'Cash' => 'fa-light fa-Money-bill',
                                                                        'Cheque' => 'fa-light fa-money-check',
                                                                        default => 'fa-light fa-Money-bill',
                                                                    };
                                                                };
                                                                ...
                                                                $students = DB::table('client_students as cs')
                                                                    ...
                                                                    ->where('cs.client_id', $q->id)
                                                                    ->groupBy('cs.student_name')
                                                                    ->orderBy('cs.student_name', 'asc')
                                                                    ->get();
                                                                $client_payments = DB::table('client_payments')
...
<div class="students-box">
    <div class="titillium-web-semibold text-dark font-8pt mb-1">
        Students
    </div>';
        ...
        <div class="student-row">
            ...
            <span class="student-namee">
                ' .
                                                                            e($s->student_name) .
                                                                            '
            </span>

            <span class="student-amount">
                $' .
                                                                    ...
                                                                    $cvm .= '
    <span class="text-darkgrey titillium-web-light font-8pt">
        None
    </span>';
                                                                }
...
<div class="students-box">
    <div class="titillium-web-semibold text-dark font-8pt mb-1">
        Payments
                                                                ...
                                                                if ($client_payments->count()) {
                                                                    foreach ($client_payments as $c) {
                                                                        $icon = $paymentIcon($c->payment_type);
    ...
    <span class="text-darkgrey titillium-web-light font-8pt">
        None
                                                            ...
                                                            <div class="d-flex">
                                                                ...
                                                                <a href="javascript:;" class="banner-icon ml-1" data-toggle="tooltip"
                                                                    data-trigger="hover" data-placement="top"
                                                                    data-title="{{ $cvm }}" data-html="true"
                                                                    data-original-title="{{ $cvm }}">
                                                                    <i
                                                                        class="fa-thin fa-graduation-cap bubbles-icon-color fs-20 regular-icon"></i>
                                                                    <i class="fa-solid fa-graduation-cap bubbles-icon-color fs-20 solid-icon"
                                                                        style="padding-right: 3px; padding-left: 4px;"></i>
                                                                </a>

                                                                <a href="javascript:;" class="banner-icon ml-1 bubble-server-icon"
                                                                    data-toggle="tooltip" data-trigger="hover" data-placement="top"
                                                                    data-title="{{ $cvm2 }}" data-html="true"
                                                                    data-original-title="{{ $cvm2 }}">
                                                                    <i class="fa-thin fa-file-invoice bubbles-icon-color fs-20"></i>
                                                                </a>
                                                                ...
                                                                <div class="dropdown-menu"
                                                                    ...
                                                                    <?php   
                    if($q->is_active){?>
                                                                    <a class="dropdown-item d-flex align-items-center pl-0 btnEnd"
                                                                        data="{{ $q->id }}" data-id="{{ $q->id }}"
                                                                        href="javascript:;" data-ended="0" data-status="Active">
                                                                        <i class="fa-light fa-octagon-exclamation mx-2 icons-3dot"></i>
                                                                        Deactivate
                                                                    </a>
                                                                    <?php }
                    else{
                                ...
                                ?>
                                                                    <a class="dropdown-item d-flex align-items-center pl-0 btnEnd  "
                                                                        data="{{ $q->id }}" data-status="{{ $q->is_active }}"
                                                                        href="javascript:;" data-id="{{ $q->id }}"
                                                                        data-ended="1" data-status="Inactive">
                                                                        <i class="fa-light fa-arrow-up-to-arc mx-2 icons-3dot"></i>
                                                                        Activate
                                                                    </a>
                                                                    <?php }?>
                                        ...
                                        <div class="col-md-12 bg-nav-tab position-relative">
                                            <nav class="align-items-center d-flex justify-content-between read-nav-tabs sub-nav-tabs">
                                                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                                    <button class="nav-link active" id="nav-main-tab-client" data-toggle="tab"
                                                        data-target="#nav-main-client" type="button" role="tab"
                                                        aria-controls="nav-main-client" aria-selected="true">Main</button>
                                                    <button class="nav-link" id="nav-students-tab-client" data-toggle="tab"
                                                        data-target="#nav-students-client" type="button" role="tab"
                                                        aria-controls="nav-students-client" aria-selected="false">Students</button>
                                                    <button class="nav-link" id="nav-payments-tab-client" data-toggle="tab"
                                                        data-target="#nav-payments-client" type="button" role="tab"
                                                        aria-controls="nav-payments-client" aria-selected="false">Payments</button>
                                                    <button class="nav-link" id="nav-vacation-tab-client" data-toggle="tab"
                                                        data-target="#nav-vacation-client" type="button" role="tab"
                                                        aria-controls="nav-vacation-client" aria-selected="false">Vacations</button>
                                        ...
                                        <div class="col-md-12 bg-edit-tab edit-sub-header">
                                            <nav class="sub-nav-tabs edit-nav-tabs d-no ne">
                                                <div class="nav nav-tabs" id="nav-tab-edit" role="tablist">
                                                    <button class="nav-link active" id="nav-main-tab-client-edit" data-toggle="tab"
                                                        data-target="#nav-main-client-edit" type="button" role="tab"
                                                        aria-controls="nav-main-client-edit" aria-selected="true">Main</button>
                                                    <button class="nav-link" id="nav-students-tab-client-edit" data-toggle="tab"
                                                        data-target="#nav-students-client-edit" type="button" role="tab"
                                                        aria-controls="nav-students-client-edit"
                                                        aria-selected="false">Students</button>
                                                    <button class="nav-link" id="nav-payments-tab-client-edit" data-toggle="tab"
                                                        data-target="#nav-payments-client-edit" type="button" role="tab"
                                                        aria-controls="nav-payments-client-edit"
                                                        aria-selected="false">Payments</button>
                                                    <button class="nav-link" id="nav-vacations-tab-client-edit" data-toggle="tab"
                                                        data-target="#nav-vacations-client-edit" type="button" role="tab"
                                                        aria-controls="nav-vacations-client-edit"
                                                        aria-selected="false">Vacations</button>
                                                    <button class="nav-link" id="nav-comments-tab-client-edit" data-toggle="tab"
                                                        data-target="#nav-comments-client-edit" type="button" role="tab"
                                                        aria-controls="nav-comments-client-edit"
                                                        aria-selected="false">Comments</button>
                                                    <button class="nav-link border-right-0" id="nav-attachments-tab-client-edit"
                                                        data-toggle="tab" data-target="#nav-attachments-client-edit" type="button"
                                                        role="tab" aria-controls="nav-attachments-client-edit"
                                                        aria-selected="false">Attachments</button>
                                                </div>
                                            </nav>
                                        </div>
                                    </div>
                                    <div class="d-none" id="showEditData" style="overflow-y: auto;height:77vh;"></div>
                                    ...
                                    <div class="modal-content">
                                        <div class="block  block-transparent mb-0">
                                            ...
                                            <div class="block-content block-content-full   pt-4"
                                                ...
                                                <button type="submit" class="btn mr-3 btn-new"
                                                    name="advance_search">Apply</button>
                                                <button type="button" class="btn     btn-new-secondary"
                                                    data-dismiss="modal">Cancel</button>
                                                @if (isset($_GET['advance_search']))
                                                    <a href="{{ url('clients') }}" class="btn     btn-new-secondary float-right"
                ...
                <div class="modal fade" id="deleteClientModal" tabindex="-1" role="dialog"
                    aria-labelledby="deleteClientModalLabel" aria-hidden="true" data-bs-backdrop="static">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <form id="deleteClientForm" class="mb-0" method="POST" action="{{ url('delete-contract') }}">
                                @csrf
                                <div class="modal-header align-items-center border-0">
                                    <h5 class="modal-title font-titillium fw-800 text-header-blue" style="font-size: 18pt;">Delete
                                    </h5>
                                ...
                                <div class="modal-body py-0">
                                    ...
                                    <div class="d-flex align-items-center">
                                        <i class="fa-light fa-triangle-exclamation text-warning fs-30 mr-2"></i>
                                        <span class="font-titillium text-darkgrey" style="font-size: 14pt;">Are you sure you wish to
                                            delete this Client?</span>
                                    </div>
                                    <hr>
                                </div>
                                <div class="modal-footer border-0 pt-0" style="justify-content: space-evenly;">
                                    <button type="button" id="deleteClientBtn" class="btn del-btn btn-danger">Delete</button>
                                    <button type="button" class="btn cancel-btn" data-dismiss="modal">Cancel</button>
                                </div>
                            </form>
                        </div>
                ...
                <div class="modal fade" id="notifyEmailsModal" tabindex="-1" role="dialog"
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header align-items-center border-0 px-4">
                                <h5 class="modal-title font-titillium fw-800 text-header-blue" style="font-size: 18pt;">Sending
                                    Notification
                                </h5>
                            </div>
                            <div class="modal-body pt-0 pb-4 px-4">
                ...
                <div class="modal fade" id="receiptPromptModalView" tabindex="-1" role="dialog"
                    aria-labelledby="receiptPromptModalViewLabel" aria-hidden="true" data-bs-backdrop="static">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header align-items-center border-0">
                                <h5 class="modal-title font-titillium fw-800 text-header-blue" style="font-size: 18pt;">Print
                                    Receipt</h5>
                            </div>
                            <div class="modal-body py-0">
                                <div class="d-flex align-items-center">
                                    <i class="fa-light fa-triangle-exclamation text-warning fs-30 mr-2"></i>
                                    <span class="font-titillium text-darkgrey" style="font-size: 14pt;">Would you like to print or
                                        email a client receipt?</span>
                                </div>
                                <hr>
                            </div>
                            <div class="modal-footer border-0 pt-0" style="justify-content: space-evenly;">
                                <button type="button" class="btn ntfy-btn btn-primary receipt-print">Print</button>
                                <button type="button" class="btn ntfy-btn btn-warning receipt-email text-black">Email</button>
                                <button type="button" class="btn cancel-btn" data-dismiss="modal">No</button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal fade" id="taxReceiptModalView" tabindex="-1" role="dialog"
                    aria-labelledby="taxReceiptModalViewLabel" aria-hidden="true" data-bs-backdrop="static">
                    <div class="modal-dialog modal-dialog-centered modal-bac" role="document">
                        <div class="modal-content">
                            <div class="modal-header align-items-center border-0 px-4">
                                <h5 class="modal-title font-titillium fw-800 text-header-blue" style="font-size: 18pt;">Tax
                                    Receipt</h5>
                            </div>
                            <div class="modal-body pt-0 px-4">
                                <div class="row">
                                    <div class="col-lg-12 mb-3">
                                        <div class="tax-receipt-panel">
                                            <div class="font-titillium text-darkgrey fs-14 mb-3">Validate which students to include
                                                in the tax receipt:</div>
                                            <div class="border p-2 mb-2 border-style edit-border" style="max-width:320px;">
                                                <h6 class="font-titillium text-grey mb-2 fw-700 pl-1">Tax Year</h6>
                                                <div class="d-flex align-items-center">
                                                    <i class="fa-light fa-calendar text-grey fs-18"></i>
                                                    <input type="number"
                                                        class="form-control font-titillium text-grey fw-300 border-0 p-0 fs-18 ml-2 edit-field"
                                                        id="taxReceiptYear" min="2000" max="2100">
                                                </div>
                                            </div>
                                            <div class="border px-2 mb-2 border-style small-box-no-arrow">
                                                <table
                                                    class="table table-sm table-striped table-borderless tax-receipt-students-table mb-0">
                                                    <thead>
                                                        <tr>
                                                            <th width="5%"></th>
                                                            <th class="font-titillium fs-13 text-grey fw-700">Student</th>
                                                            <th class="font-titillium fs-13 text-grey fw-700 text-right tax-receipt-amount-head">Amount</th>
                                                            <th width="15%"></th>
                                                        </tr>
                                                    </thead>
                                                    <tbody id="taxReceiptStudentsBody"></tbody>
                                                </table>
                                            </div>
                                            {{-- <div class="border p-2 mb-3 border-style edit-border">
                                                <h6 class="font-titillium text-grey mb-2 fw-700 pl-1">Memo</h6>
                                                <textarea id="taxReceiptMemo" rows="4"
                                                    class="form-control font-titillium text-grey fw-300 border-0 p-0 fs-14 edit-field"></textarea>
                                            </div> --}}
                                            <div class="border p-2 mb-3 border-style edit-border">
                                                <h6 class="font-titillium text-grey mb-2 fw-700 pl-1">Memo</h6>
                                                <div class="d-flex pb-1 pl-1 pt-0">
                                                    <i class="fa-light fa-memo text-grey fs-18 constant-icon"></i>
                                                    <textarea id="taxReceiptMemo" rows="4"
                                                    class="form-control font-titillium text-grey fw-300 border-0 p-0 fs-14 edit-field  ml-2"></textarea>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="modal-footer border-0 pt-0" style="justify-content: space-evenly;">
                                <button type="button" class="btn ok-btn btn-primary" id="taxReceiptPrintBtn">Print</button>
                                <button type="button" class="btn ntfy-btn btn-warning text-black"
                                    id="taxReceiptEmailBtn">Email</button>
                                <button type="button" class="btn cancel-btn" data-dismiss="modal">Cancel</button>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal fade" id="taxReceiptAmountModal" tabindex="-1" role="dialog"
                    aria-labelledby="taxReceiptAmountModalLabel" aria-hidden="true" data-bs-backdrop="static">
                    <div class="modal-dialog modal-dialog-centered modal-bac" role="document" style="max-width: 460px;">
                        <div class="modal-content">
                            <div class="modal-header align-items-center border-0 px-4 pb-2">
                                <h5 class="modal-title font-titillium fw-800 text-header-blue fs-20" id="taxReceiptAmountModalLabel">
                                    Modify Taxable Amount
                                </h5>
                                <button type="button" class="close text-darkgrey" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true"><i class="fa-light fa-circle-xmark"></i></span>
                                </button>
                            </div>
                            <div class="modal-body pt-0 px-4">
                                <div class="font-titillium text-darkgrey fs-18 mb-2" id="taxReceiptAmountStudentName"></div>
                                <div class="border p-2 mb-3 border-style edit-border">
                                    <h6 class="font-titillium text-grey mb-2 fw-700 pl-1">Taxable Amount</h6>
                                    <div class="d-flex align-items-center">
                                        <i class="fa-light fa-money-check-dollar text-grey fs-18"></i>
                                        <input type="text"
                                            class="form-control font-titillium text-grey fw-300 border-0 p-0 fs-18 ml-2 edit-field"
                                            id="taxReceiptAmountInput">
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer border-0 pt-0" style="justify-content: flex-end;">
                                <button type="button" class="btn ok-btn btn-primary" id="taxReceiptAmountSaveBtn">OK</button>
                            </div>
                        </div>
                    </div>
                </div>
                {{-- <div id="taxReceiptPrintable" class="d-none"> --}}
                <div id="taxReceiptPrintable">
                    <div class="tax-receipt-preview" style="max-width:794px;margin:0 auto;padding:30px 34px;">
                        <div style="text-align:center;font-size:28px;font-weight:800;line-height:1.1;margin-bottom:14px;">RECEIPT /
                            RECU</div>
                        <div style="display:flex;justify-content:space-between;align-items:flex-start;line-height:1.45;">
                            <div style="width:57%;font-size:16px;">
                                <div id="taxReceiptCenterName" style="font-size:20px;font-weight:700;margin-bottom:4px;"></div>
                                <div id="taxReceiptCenterAddress"></div>
                                <div id="taxReceiptCenterCity"></div>
                                <div id="taxReceiptCenterPostal"></div>
                                <div style="margin-top:6px;"><strong>Tel:</strong> <span id="taxReceiptCenterTelephone"></span>
                                </div>
                                <div><strong>Email:</strong> <span id="taxReceiptClientEmail"></span></div>
                            </div>
                            <div style="width:40%;font-size:16px;">
                                <div id="taxReceiptNumber" style="font-size:20px;font-weight:800;line-height:1;">#002000</div>
                                <div style="font-size:13px;color:#666;">No. de Recu / Receipt No</div>
                                <div style="margin-top:16px;"><strong>Date:</strong> <span id="taxReceiptDate"></span></div>
                                <div><strong>Client:</strong> <span id="taxReceiptClientName"></span></div>
                            </div>
                        </div>

                        <hr style="margin:18px 0 14px;">

                        <div style="font-size:17px;font-weight:700;margin-bottom:5px;">Memo</div>
                        <div id="taxReceiptPreviewMemo" style="font-size:16px;line-height: 2.5;margin-bottom:14px;"></div>

                        <div style="font-size:17px;font-weight:700;margin-bottom:6px;">Etudiants / Students</div>
                        <div style="border: 1px solid #747474;border-radius:8px;padding: 5px;">
                            <table
                                style="width:100%;border-collapse:separate;border-spacing:0;border-radius:8px;overflow:hidden;font-size:15px;">
                                <thead>
                                    <tr>
                                        <th style="padding: 10px;">
                                            <h6
                                                style="padding:4px 0px;text-align:left;border-right:1px solid #747474; font-size: 16px; margin: 0px;">
                                                ID</h6>
                                        </th>
                                        <th style="padding: 10px;">
                                            <h6
                                        ...
                                        <th style="padding: 10px;">
                                            <h6 style="padding:4px 0px;text-align:right; font-size: 16px; margin: 0px;">Montant /
                                                Amount</h6>
                                        </th>
                                    </tr>
                                </thead>
                                <tbody id="taxReceiptPreviewStudents"></tbody>
                            </table>
                        </div>

                        <div style="display:block;text-align:right;margin-top:12px;font-weight:700;">
                            <span style="font-size: 20px;margin-right: 14px;">TOTAL:</span>
                            <span id="taxReceiptPreviewTotal" style="font-size:20px;"></span>
                        </div>
                    </div>
                </div>
                <div class="modal fade" id="taxReceiptProcessingModal" tabindex="-1" role="dialog"
                    aria-labelledby="taxReceiptProcessingModalLabel" aria-hidden="true" data-bs-backdrop="static">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header align-items-center border-0 px-4">
                                <h5 class="modal-title font-titillium fw-800 text-header-blue" id="taxReceiptProcessingTitle"
                                    style="font-size: 18pt;">Processing</h5>
                            </div>
                            <div class="modal-body pt-0 pb-4 px-4">
                                <div class="d-flex align-items-center">
                                    <i class="fa-light fa-gear-complex fa-spin text-darkgrey fs-30 mr-2"></i>
                                    <span class="font-titillium text-darkgrey" id="taxReceiptProcessingText"
                                        style="font-size: 14pt;">Please wait while processing...</span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="modal fade" id="centerSettingsModal" tabindex="-1" role="dialog"
                    aria-labelledby="centerSettingsModalLabel" aria-hidden="true" data-bs-backdrop="static">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header align-items-center border-0 px-4 position-relative">
                                <h5 class="modal-title font-titillium fw-800 text-header-blue" style="font-size: 18pt;">Center
                                    Settings</h5>
                                <button type="button" class="close" data-dismiss="modal">
                                    <i class="fa-solid fa-circle-xmark"></i>
                                </button>
                            ...
                            <div class="modal-body pt-0 px-4">
                                <div class="border p-2 mb-3 border-style edit-border position-relative">
                                    <a href="javascript:void();" class="mandatory-icon position-absolute"
                                        style="right: 9px; top: 9px;" data-toggle="tooltip" data-trigger="hover"
                                        data-placement="top" title="" data-original-title="Mandatory"><i
                                            class="fa-light fa-circle-star text-grey fs-20"></i></a>
                                    {{-- <h6 class="font-titillium text-grey mb-2 fw-700 pl-1">Administrator Email</h6> --}}
                                    <div class="d-flex align-items-center justify-content-between mb-2">
                                        <h6 class="font-titillium text-grey mb-0 fw-700 pl-1">Administrator Email</h6>
                                        <button type="button" class="btn banner-icon affiliate-info ml-auto p-0 cursor-help"
                                            data-toggle="tooltip" data-trigger="hover" data-placement="top" title=""
                                            data-original-title="" style="margin-right: 30px;">
                                            <i class="fa-light fa-circle-question text-grey fs-20 regular-icon"></i>
                                            <i class="fa-solid fa-circle-question text-primary fs-20 header-solid-icon"></i>
                                        </button>
                                        <div class="affiliate-tooltip font-titillium text-grey fw-300">
                                            <strong class="titillium-web-black fs-18 text-primary" style="line-height:1.6;">Kumon
                                                Administrator Email</strong><br>
                                            This email will receive the following email
                                            notifications generated by the system: <br>
                                            - Payment method changes<br>
                                            - Student price changes<br>
                                            - New vacation requires planning
                                        </div>
                                    </div>
                                    <div class="d-flex pb-1 pl-1 pt-0">
                                ...
                                <div class="border p-2 mb-3 border-style edit-border position-relative">
                                    <a href="javascript:void();" class="mandatory-icon position-absolute" style="right: 9px;"
                                        data-toggle="tooltip" data-trigger="hover" data-placement="top" title=""
                                        data-original-title="Mandatory"><i class="fa-light fa-circle-star text-grey fs-20"></i></a>
                                    <h6 class="font-titillium text-grey mb-2 fw-700 pl-1">Center Name</h6>
                                    <div class="d-flex pb-1 pl-1 pt-0">
                                        <i class="fa-light fa-building text-grey fs-18 constant-icon"></i>
                                        <input type="text"
                                            class="form-control font-titillium text-grey fw-300 mb-0 ml-2 border-0 p-0 fs-18 rounded-0 edit-field"
                                            id="center_name">
                                    </div>
                                ...
                                <div class="border p-2 mb-3 border-style edit-border position-relative">
                                    ...
                                    <div class="d-flex pb-1 pl-1 pt-0">
                                        <i class="fa-light fa-map text-grey fs-18 constant-icon"></i>
                                        <input type="text"
                                            class="form-control font-titillium text-grey fw-300 mb-0 ml-2 border-0 p-0 fs-18 rounded-0 edit-field"
                                            id="center_province">
                                    </div>
                                ...
                                <div class="border p-2 mb-3 border-style edit-border position-relative">
                                    ...
                                    <div class="d-flex pb-1 pl-1 pt-0">
                                        <i class="fa-light fa-mailbox text-grey fs-18 constant-icon"></i>
                                        <input type="text"
                                            class="form-control font-titillium text-grey fw-300 mb-0 ml-2 border-0 p-0 fs-18 rounded-0 edit-field"
                                ...
                                <div class="border p-2 mb-3 border-style edit-border position-relative">
                                    <a href="javascript:void();" class="mandatory-icon position-absolute" style="right: 9px;"
                                        data-toggle="tooltip" data-trigger="hover" data-placement="top" title=""
                                        data-original-title="Mandatory"><i class="fa-light fa-circle-star text-grey fs-20"></i></a>
                                    <h6 class="font-titillium text-grey mb-2 fw-700 pl-1">Telephone #</h6>
                                    <div class="d-flex pb-1 pl-1 pt-0">
                                        <i class="fa-light fa-phone text-grey fs-18 constant-icon"></i>
                                        <input type="text"
                                            class="form-control font-titillium text-grey fw-300 mb-0 ml-2 border-0 p-0 fs-18 rounded-0 edit-field"
                                            id="center_telephone">
                                    </div>
                            ...
                            <div class="modal-footer border-0 pt-0" style="justify-content: space-evenly;">
                                <button type="button" class="btn ok-btn btn-primary" id="saveCenterSettings">Save</button>
                                <button type="button" class="btn cancel-btn" data-dismiss="modal">Cancel</button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal fade" id="printReceiptModal" tabindex="-1" role="dialog"
                    aria-labelledby="printReceiptModalLabel" aria-hidden="true" data-bs-backdrop="static">
                    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                        <div class="modal-content">
                            ...
                            <div class="modal-body pt-0 px-4">
                                <div id="printReceiptPreview" class="border p-3" style="background:#fff;">
                                    <div style="font-family: Arial, sans-serif; color: #3f3f3f;">
                                        <p>Dear <span id="receiptClientName"></span>,</p>
                                        ...
                                        <table cellpadding="6" cellspacing="0" border="0"
                                            ...
                                            <tr>
                                                ...
                                                <td id="receiptPaymentMonth"></td>
                                            </tr>
                                            <tr>
                                                <td><strong>Amount</strong></td>
                                                <td id="receiptPaymentAmount"></td>
                                            </tr>
                                            <tr>
                                                <td><strong>Reference No.</strong></td>
                                                <td id="receiptPaymentReference"></td>
                                            </tr>
                                        </table>
                                        <p>Kind regards,</p>
                            ...
                            <div class="modal-footer border-0 pt-0" style="justify-content: space-evenly;">
                                <button type="button" class="btn ok-btn btn-primary" id="printReceiptBtn">Print</button>
                ...
                <div class="modal fade" id="importModal" tabindex="-1" role="dialog" aria-labelledby="importModalLabel"
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header align-items-center border-0 px-4">
                                <h5 class="modal-title font-titillium fw-800 text-header-blue" style="font-size: 18pt;">Import</h5>
                                <button type="button" class="close" data-dismiss="modal">
                                    <i class="fa-solid fa-circle-xmark"></i>
                                </button>
                            </div>
                            <div class="modal-body pt-0 px-4">
                                <form id="importForm" enctype="multipart/form-data">
                                    @csrf
                                    <div class="border p-2 mb-3 border-style edit-border">
                                        <h6 class="font-titillium text-grey mb-2 fw-700 pl-1">Import Type</h6>
                                        <select class="form-control font-titillium text-grey fw-300 border-0 p-0 fs-18 edit-field"
                                            name="type" id="importType">
                                            <option value="clients">Clients</option>
                                            <option value="students">Students</option>
                                        </select>
                                    </div>
                                    <div class="border p-2 mb-3 border-style edit-border">
                                        <h6 class="font-titillium text-grey mb-2 fw-700 pl-1">CSV File</h6>
                                        <input type="file"
                                            class="form-control font-titillium text-grey fw-300 border-0 p-0 fs-18 edit-field"
                                            name="file" id="importFile" accept=".csv">
                                    </div>
                                </form>
                            </div>
                            <div class="modal-footer border-0 pt-0" style="justify-content: space-evenly;">
                                <button type="button" class="btn ok-btn btn-primary" id="confirmImport">Import</button>
                                <button type="button" class="btn cancel-btn" data-dismiss="modal">Cancel</button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal fade" id="exportProgressModal" tabindex="-1" role="dialog"
                    aria-labelledby="SaveModalLabel" aria-hidden="true" data-bs-backdrop="static">
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header align-items-center border-0 px-4">
                                <h5 class="modal-title font-titillium fw-800 text-header-blue" style="font-size: 18pt;">Exporting
                                </h5>
                            </div>
                            <div class="modal-body pt-0 pb-4 px-4">
                                <div class="d-flex align-items-center">
                                    <i class="fa-light fa-gear-complex fa-spin text-darkgrey fs-30 mr-2"></i>
                                    <span class="font-titillium text-darkgrey" style="font-size: 14pt;">Please wait while the export
                                        is in progress...</span>
                                </div>
                            </div>
                        </div>
                ...
                <div class="modal fade" id="centerReceiptModal" tabindex="-1" role="dialog"
                    aria-labelledby="centerReceiptModalLabel" aria-hidden="true" data-bs-backdrop="static">
                    <div class="modal-dialog modal-lg modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header align-items-center border-0 px-4">
                                <h5 class="modal-title font-titillium fw-800 text-header-blue" style="font-size: 18pt;">Print
                            ...
                            <div class="modal-body pt-0 px-4">
                                <div id="centerReceiptPreview" class="border p-3"
                                    style="
                        background:#fff;
                        width:100%;
                    ...
                    ">
                        ...
                        ">
                                        <img src="{{ asset('public/logo/kumon-logo.png') }}" alt="Kumon"
                            ...
                            ">
                                ...
                                ">
                                            ...
                                            <div id="centerReceiptAddress"
                                                style="
                                    font-weight:500;
                                ">
                                            </div>

                                            <div id="centerReceiptCity"></div>

                                            <div id="centerReceiptTelephone"
                                                style="
                                    font-weight:500;
                                ">
                                            </div>
                        ...
                        ">
                                        ...
                                        <div id="centerReceiptClientName"
                                            style="
                                font-size:16px;
                                font-weight:600;
                                margin-top:2px;
                            ">
                                        ...
                                        <div id="centerReceiptClientAddress"
                                            style="
                                font-weight:500;
                            ">
                                        </div>
                        ...
                        ">
                                ...
                                ">
                                    ...
                                    width:45%;
                                "
                                                id="centerReceiptPaymentMethod"></td>
                                        </tr>
                                ...
                                ">
                                    ...
                                    padding:6px 0;
                                "
                                                id="centerReceiptPaymentDate"></td>
                                        </tr>
                                ...
                                ">
                                    ...
                                    padding:6px 0;
                                "
                                                id="centerReceiptPaymentMonth"></td>
                                        </tr>
                                ...
                                ">
                                    ...
                                    font-weight:700;
                                "
                                                id="centerReceiptPaymentAmount"></td>
                                        </tr>
                                ...
                                ">
                                    ...
                                    padding:6px 0;
                                "
                                                id="centerReceiptPaymentReference"></td>
                                        </tr>
                                    </table>
                                </div>
                            ...
                            <div class="modal-footer border-0 pt-0" style="justify-content: space-evenly;">
                                <button type="button" class="btn ok-btn btn-primary" id="printCenterReceiptBtn">Print</button>
                                <button type="button" class="btn cancel-btn" data-dismiss="modal">Close</button>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal fade" id="exportModal" tabindex="-1" role="dialog" aria-labelledby="exportModalLabel"
                    ...
                    <div class="modal-dialog modal-dialog-centered" role="document">
                        <div class="modal-content">
                            <div class="modal-header align-items-center border-0 px-4">
                                <h5 class="modal-title font-titillium fw-800 text-header-blue" style="font-size: 18pt;">Export</h5>
                                <button type="button" class="close" data-dismiss="modal">
                                    <i class="fa-solid fa-circle-xmark"></i>
                                </button>
                            </div>
                            <div class="modal-body pt-0 px-4">
                                <div class="border p-2 mb-3 border-style edit-border">
                                    <h6 class="font-titillium text-grey mb-2 fw-700 pl-1">Export Type</h6>
                                    <select class="form-control font-titillium text-grey fw-300 border-0 p-0 fs-18 edit-field"
                                        id="exportType">
                                        <option value="clients">Clients</option>
                                        <option value="students">Students</option>
                            ...
                            <div class="modal-footer border-0 pt-0" style="justify-content: space-evenly;">
                                <button type="button" class="btn ok-btn btn-primary" id="confirmExport">Export</button>
                                <button type="button" class="btn cancel-btn" data-dismiss="modal">Cancel</button>
                            </div>
                        </div>
                    </div>
                    ...
                    $(function() {
                        ...
                        if (typeof window.showError !== 'function') {
                            window.showError = function(message) {
                                if (window.Dashmix && typeof Dashmix.helpers === 'function') {
                        ...
                        $(document).on('click', '.reciept-btn', async function() {
                            var payment_date = $(this).attr('data-payment_date');
                            var kumon_month = $(this).attr('data-kumon_month');
                            var payment_type = $(this).attr('data-payment_type');
                            var amount = $(this).attr('data-amount');
                            var reference_no = $(this).attr('data-reference_no');

                            try {
                                await getClientData(); // NOW it actually waits
                                showReceiptPrompt(payment_date, kumon_month, payment_type, amount, reference_no);
                            } catch (e) {}
                        });

                        function showReceiptPrompt(payment_date,
                            kumon_month,
                            payment_type,
                            amount,
                            reference_no) {
                            $('#receiptPromptModalView').attr('data-payment_date', payment_date);
                            $('#receiptPromptModalView').attr('data-kumon_month', kumon_month);
                            $('#receiptPromptModalView').attr('data-payment_type', payment_type);
                            $('#receiptPromptModalView').attr('data-amount', amount);
                            $('#receiptPromptModalView').attr('data-reference_no', reference_no);
                            $('#receiptPromptModalView').modal('show');
                        }

                        let currentClientData = {};
                        let latestPaymentData = {};
                        let centerSettingsData = {};
                        ...
                        function formatTaxCurrency(value) {
                        ...
                        function formatTaxAmountNoSymbol(value) {
                            return formatTaxCurrency(value).replace(/\s*\$$/, '');
                        }

                        function parseTaxAmountInput(value) {
                        ...
                        function getDateDDMonYYYY(dateObj) {
                        ...
                        async function reserveTaxReceiptNumber() {
                            const res = await $.ajax({
                                type: 'post',
                                url: '{{ url('reserve-tax-receipt-number') }}',
                                headers: {
                                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                                }
                            });
                            if (!res || !res.success || !res.number) {
                                throw new Error((res && res.message) ? res.message : 'Unable to reserve receipt number.');
                            }
                            return String(res.number);
                        }

                        function collectTaxReceiptStudentsFromReadTable() {
                            const rows = $('#nav-students-client .student-table tbody tr');
                            const students = [];
                            rows.each(function() {
                                const $row = $(this);
                                ...
                                const subjectText = $row.find('.subject-text').text().trim();
                                const subjects = subjectText || '-';
                                const amountMonthly = parseMoneyValue($row.find('.amount').text());
                                const isActive = $row.find('.student-status .fa-circle-check').length > 0;
                                students.push({
                        ...
                        function buildDefaultTaxMemo(year, selectedStudents) {
                            const allSubjects = selectedStudents.map(s => String(s.subjects || '').toLowerCase()).join(' ');
                            const hasMath = allSubjects.includes('math');
                            const hasReading = allSubjects.includes('reading');
                            const hasEfl = allSubjects.includes('efl') || allSubjects.includes('english');
                            const labels = [];
                            if (hasMath) labels.push('mathematics');
                            if (hasReading) labels.push('reading');
                            if (hasEfl) labels.push('english');
                            const enSubjects = labels.length ? labels.join(' and ') : 'education';
                            const frSubjects = labels.length ? labels.join(' et ') : 'education';
                            return `Programme parascolaire de ${frSubjects} pour ${year}\nExtracurricular program in ${enSubjects} for ${year}`;
                        }

                        function syncTaxMemoWithYearIfDefault() {
                            const currentYear = Number(taxReceiptState.year) || ((new Date()).getFullYear() - 1);
                            const inputYear = parseInt($('#taxReceiptYear').val(), 10);
                            const nextYear = Number.isFinite(inputYear) ? inputYear : currentYear;
                            const currentMemo = String($('#taxReceiptMemo').val() || '').trim();
                            const defaultMemoForCurrentYear = buildDefaultTaxMemo(currentYear, taxReceiptState
                                .students).trim();

                            if (currentMemo === defaultMemoForCurrentYear) {
                                $('#taxReceiptMemo').val(buildDefaultTaxMemo(nextYear, taxReceiptState.students));
                            }
                        }

                        function renderTaxReceiptStudents() {
                            const $body = $('#taxReceiptStudentsBody');
                            $body.empty();
                            taxReceiptState.students.forEach((student, index) => {
                                const statusIcon = student.status === 'active' ?
                                    '<i class="fa-light fa-circle-check text-green fs-16" title="Active"></i>' :
                                    '<i class="fa-light fa-circle-xmark text-red fs-16" title="Inactive"></i>';
                                $body.append(`
                                    <tr data-index="${index}">
                                        <td class="text-center">${statusIcon}</td>
                                        <td>
                                            <div class="font-titillium text-darkgrey fs-15">${student.student_name}</div>
                                        </td>
                                        <td class="text-right tax-receipt-amount-cell">
                                            <div class="font-titillium text-darkgrey fs-15">${formatTaxCurrency(student.amount)}</div>
                                        </td>
                                        <td class="text-right">
                                            <a href="javascript:;" class="tax-receipt-edit mr-1" data-index="${index}" data-student="" style="opacity:0;pointer-events:none;">
                                                <i class="fa-light fa-pencil text-darkgrey"></i>
                                            </a>
                                            <a href="javascript:;" class="tax-receipt-remove" data-index="${index}" style="opacity:0;pointer-events:none;">
                                                <i class="fa-light fa-circle-xmark text-darkgrey"></i>
                                            </a>
                                        </td>
                                    </tr>
                                `);
                        ...
                        function renderTaxReceiptPreview() {
                            const memo = String($('#taxReceiptMemo').val() || '').trim();
                            const yearValue = parseInt($('#taxReceiptYear').val(), 10);
                            const year = Number.isFinite(yearValue) ? yearValue : (taxReceiptState.year || ((new Date())
                                .getFullYear() - 1));
                            taxReceiptState.year = year;
                            $('#taxReceiptPreviewMemo').html((memo || '').replace(/\n/g, '<br>'));
                            const $previewRows = $('#taxReceiptPreviewStudents');
                            $previewRows.empty();
                            let total = 0;
                            taxReceiptState.students.forEach((student, index) => {
                                const amount = Number(student.amount) || 0;
                                total += amount;
                                const rowBg = (index % 2 === 0) ? '#eef3f9' : '#ffffff';
                                const isBlueRow = rowBg.toLowerCase() === '#eef3f9';
                                const leftRadiusStyle = isBlueRow ? 'border-top-left-radius:10px;border-bottom-left-radius:10px;' : '';
                                const rightRadiusStyle = isBlueRow ? 'border-top-right-radius:10px;border-bottom-right-radius:10px;' : '';
                                $previewRows.append(`
                                    <tr style="background:${rowBg};">
                                        <td style="padding:8px 10px;${leftRadiusStyle}">${student.student_id || ''}</td>
                                        <td style="padding:8px 10px;">${student.student_name || ''}</td>
                                        <td style="padding:8px 10px;">${student.subjects || ''}</td>
                                        <td style="padding:8px 10px;text-align:right;${rightRadiusStyle}">${formatTaxCurrency(amount)}</td>
                                    </tr>
                                `);
                            });
                            $('#taxReceiptPreviewTotal').text(formatTaxCurrency(total));

                            const clientName = `${currentClientData.lastname || ''}, ${currentClientData.firstname || ''}`
                                .replace(/^,\s*/, '').trim();
                            const cityProvince = [centerSettingsData.city || '', centerSettingsData.province || '']
                                .filter(Boolean).join(', ');
                            $('#taxReceiptCenterName').text(centerSettingsData.center_name || '');
                            $('#taxReceiptCenterAddress').text(centerSettingsData.address || '');
                            $('#taxReceiptCenterCity').text(cityProvince);
                            $('#taxReceiptCenterPostal').text(centerSettingsData.postal_code || '');
                            $('#taxReceiptCenterTelephone').text(centerSettingsData.telephone || '');
                            $('#taxReceiptClientEmail').text(centerSettingsData.administrator_email || '-');
                            $('#taxReceiptDate').text(getDateDDMonYYYY(new Date()));
                            $('#taxReceiptClientName').text(clientName || 'Client');
                            $('#taxReceiptNumber').text(`#${taxReceiptState.receiptNo}`);
                        }

                        async function openTaxReceiptModal() {
                            taxReceiptState.receiptNo = await reserveTaxReceiptNumber();
                            taxReceiptState.year = (new Date()).getFullYear() - 1;
                            taxReceiptState.students = collectTaxReceiptStudentsFromReadTable();
                            $('#taxReceiptYear').val(taxReceiptState.year);
                            $('#taxReceiptMemo').val(buildDefaultTaxMemo(taxReceiptState.year, taxReceiptState.students));
                            $('#taxReceiptPrintProgress, #taxReceiptEmailProgress, #taxReceiptPrintSuccess, #taxReceiptEmailSuccess')
                                .addClass('d-none');
                            renderTaxReceiptStudents();
                            renderTaxReceiptPreview();
                            $('#taxReceiptModalView').modal('show');
                        }

                        function showTaxReceiptProcessing(kind) {
                            if (kind === 'email') {
                                $('#taxReceiptProcessingTitle').text('Emailing Tax Receipt');
                                $('#taxReceiptProcessingText').text('Please wait while the emailing is in progress ...');
                            } else {
                                $('#taxReceiptProcessingTitle').text('Printing Tax Receipt');
                                $('#taxReceiptProcessingText').text('Please wait while the printing is in progress ...');
                            }
                            $('#taxReceiptProcessingModal').modal('show');
                        }

                        function hideTaxReceiptProcessing() {
                            const $modal = $('#taxReceiptProcessingModal');
                            $modal.modal('hide');
                            // Force close in case Bootstrap transition/backdrop gets stuck
                            $modal.removeClass('show').css('display', 'none').attr('aria-hidden', 'true');
                            $('body').removeClass('modal-open').css('padding-right', '');
                            $('.modal-backdrop').remove();
                        }

                        function fetchReceiptData(clientId, onDone) {
                            const activeClientId = $('.viewContent.c-active').attr('data');
                            if (!activeClientId) {
                                showError('No client selected for receipt.');
                                return;
                            }
                            $.ajax({
                                type: 'get',
                                ...
                                success: function(res) {
                                    ...
                                    latestPaymentData = res.latest_payment || {};
                                    currentClientId = res.id || activeClientId;
                                    if (typeof onDone === 'function') {
                                        onDone();
                                    }
                                },
                                error: function() {
                                    showError('Unable to load client receipt data.');
                                }
                            });
                        }

                        function fetchCenterSettings(onDone) {
                            $.ajax({
                                type: 'get',
                                url: '{{ url('center-settings') }}',
                                success: function(res) {
                                    centerSettingsData = res || {};
                                    if (typeof onDone === 'function') {
                                        onDone();
                                    }
                                },
                                error: function() {
                                    showError('Unable to load center settings.');
                                }
                            });
                        }

                        $(document).off('click.clientRead', '.tax-reciept-btn').on('click.clientRead',
                            '.tax-reciept-btn',
                            async function(e) {
                                e.preventDefault();
                                try {
                                    await loadCenterSettingsAsync();
                                    await loadReceiptClientDataAsync(currentClientId);
                                    await openTaxReceiptModal();
                                } catch (err) {
                                    console.error(err);
                                    showError((err && err.message) ? err.message :
                                        'Unable to open tax receipt.');
                                }
                            });

                        $(document).on('click.viewPage', '.tax-receipt-remove', function() {
                            const index = parseInt($(this).attr('data-index'), 10);
                            if (!Number.isInteger(index) || !taxReceiptState.students[index]) {
                                return;
                            }
                            taxReceiptState.students.splice(index, 1);
                            renderTaxReceiptStudents();
                            renderTaxReceiptPreview();
                        });

                        $(document).on('click.viewPage', '.tax-receipt-edit', function(e) {
                            e.preventDefault();
                            const index = parseInt($(this).attr('data-index'), 10);
                            if (!Number.isInteger(index) || !taxReceiptState.students[index]) {
                                return;
                            }
                            const student = taxReceiptState.students[index];
                            $('#taxReceiptAmountStudentName').text(student.student_name || '');
                            $('#taxReceiptAmountInput').val(formatTaxAmountNoSymbol(student.amount));
                            $('#taxReceiptAmountModal').data('index', index).modal('show');
                            setTimeout(function() {
                                $('#taxReceiptAmountInput').trigger('focus').select();
                            }, 120);
                        });

                        $(document).on('click.viewPage', '#taxReceiptAmountSaveBtn', function() {
                            const index = parseInt($('#taxReceiptAmountModal').data('index'), 10);
                            if (!Number.isInteger(index) || !taxReceiptState.students[index]) {
                                return;
                            }
                            const updatedAmount = parseTaxAmountInput($('#taxReceiptAmountInput').val());
                            if (!Number.isFinite(updatedAmount)) {
                                showError('Please enter a valid taxable amount.');
                                return;
                            }
                            taxReceiptState.students[index].amount = updatedAmount;
                            $('#taxReceiptAmountModal').modal('hide');
                            renderTaxReceiptStudents();
                            renderTaxReceiptPreview();
                        });

                        $(document).on('keypress.viewPage', '#taxReceiptAmountInput', function(e) {
                            if (e.which === 13) {
                                e.preventDefault();
                                $('#taxReceiptAmountSaveBtn').trigger('click');
                            }
                        });

                        $(document).on('mouseenter.viewPage', '#taxReceiptStudentsBody tr', function() {
                            $(this).find('.tax-receipt-remove').css({
                                opacity: '1',
                                pointerEvents: 'auto'
                            });
                        });

                        $(document).on('mouseleave.viewPage', '#taxReceiptStudentsBody tr', function() {
                            $(this).find('.tax-receipt-remove').css({
                                opacity: '0',
                                pointerEvents: 'none'
                            });
                        ...
                        $(document).on('mouseenter.viewPage', '#taxReceiptStudentsBody tr', function() {
                            $(this).find('.tax-receipt-edit').css({
                                opacity: '1',
                                pointerEvents: 'auto'
                            });
                        });
                        ...
                        $(document).on('keyup.viewPage change.viewPage', '#taxReceiptYear', function() {
                            syncTaxMemoWithYearIfDefault();
                            renderTaxReceiptPreview();
                        });

                        $(document).on('keyup.viewPage change.viewPage', '#taxReceiptMemo', function() {
                            renderTaxReceiptPreview();
                        });

                        $(document).on('click.viewPage', '.tax-receipt-dismiss', function() {
                        ...
                        $(document).on('click.viewPage', '#taxReceiptPrintBtn', async function() {
                            if (taxReceiptState.students.length === 0) {
                                showError('Please include at least one student.');
                                return;
                            }
                            try {
                                await loadCenterSettingsAsync();
                                await loadReceiptClientDataAsync(currentClientId);
                                renderTaxReceiptPreview();
                                $('#taxReceiptModalView').modal('hide');
                                showTaxReceiptProcessing('print');
                                setTimeout(function() {
                                    hideTaxReceiptProcessing();
                                    setTimeout(function() {
                                        printReceipt('#taxReceiptPrintable');
                                        Dashmix.helpers('notify', {
                                            type: 'success',
                                            message: `
                                            <div>
                                                <div class="font-titillium text-renue-blue" style="font-weight: 800; font-size: 15pt;">Print Receipt</div>
                                                <div class="d-flex align-items-center">
                                                    <div style="font-size: 14pt; margin-right: 8px;"><i class="fa-thin fa-envelope text-darkgrey"></i></div>
                                                    <div>Printout completed.</div>
                                                </div>
                                            </div>
                                            `,
                                            allow_dismiss: true,
                        ...
                        $(document).on('click.viewPage', '#taxReceiptEmailBtn', async function() {
                            if (taxReceiptState.students.length === 0) {
                                showError('Please include at least one student.');
                                return;
                            }
                            const email = ($('.primary-email').text() || '').trim();
                            const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                            if (!email || !emailPattern.test(email)) {
                                showError('Please enter a valid primary email address.');
                                return;
                            }
                            const total = taxReceiptState.students.reduce((sum, s) => sum + (Number(s.amount) || 0),
                                0);
                            const clientName =
                                `${currentClientData.salutation || ''} ${currentClientData.firstname || ''} ${currentClientData.lastname || ''}`
                                .replace(/\s+/g, ' ').trim();
                            renderTaxReceiptPreview();
                            const $taxReceiptEmail = $('#taxReceiptPrintable').clone();
                            $taxReceiptEmail.find('.tax-receipt-preview').attr('style', function(_, currentStyle) {
                                return `${currentStyle || ''};border:1px solid #d9d9d9;border-radius:10px;background:#fff;font-size:10pt;box-sizing:border-box;`;
                            });
                            const taxReceiptEmailHtml = $taxReceiptEmail.html();
                            $('#taxReceiptModalView').modal('hide');
                            showTaxReceiptProcessing('email');
                            try {
                                const res = await $.ajax({
                                    type: 'post',
                                    url: '{{ url('send-payment-receipt') }}',
                                    headers: {
                                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                                    },
                                    data: {
                                        ...
                                        payment_type: 'Tax Receipt',
                                        payment_date: getDateDDMonYYYY(new Date()),
                                        kumon_month: String(taxReceiptState.year),
                                        amount: total.toFixed(2),
                                        reference_no: taxReceiptState.receiptNo,
                                        subject: 'Tax Receipt',
                                        receipt_html: taxReceiptEmailHtml
                                    }
                                });
                                if (res && res.success) {
                                    Dashmix.helpers('notify', {
                                        ...
                                        message: `
                                        <div>
                                            <div class="font-titillium text-renue-blue" style="font-weight: 800; font-size: 15pt;">Email Receipt</div>
                                            <div class="d-flex align-items-center">
                                                <div style="font-size: 14pt; margin-right: 8px;"><i class="fa-thin fa-envelope text-darkgrey"></i></div>
                                    ...
                                    showError((res && res.message) ? res.message :
                                        'Unable to send tax receipt email.');
                                }
                            } catch (err) {
                                console.error(err);
                                showError('Server error sending tax receipt email.');
                            } finally {
                                hideTaxReceiptProcessing();
                            }
                        });

                        $(document).on('click.viewPage', '.receipt-print', async function() {

                            const payment_type = $('#receiptPromptModalView').attr('data-payment_type');
                            const payment_date = $('#receiptPromptModalView').attr('data-payment_date');
                            const kumon_month = $('#receiptPromptModalView').attr('data-kumon_month');
                            const amount = $('#receiptPromptModalView').attr('data-amount');
                            const reference_no = $('#receiptPromptModalView').attr('data-reference_no');

                            // keep prompt values safe
                            const promptPayment = {
                                payment_type: payment_type || '-',
                                payment_date: payment_date || '-',
                                kumon_month: kumon_month || '-',
                                amount: amount || '-',
                                reference_no: reference_no || '-'
                            };

                            $('#receiptPromptModalView').modal('hide');

                            try {
                                await loadCenterSettingsAsync();
                                await loadReceiptClientDataAsync(currentClientId);

                                // IMPORTANT: AJAX overwrote latestPaymentData, restore prompt values
                                latestPaymentData = {
                                    ...latestPaymentData,
                                    ...promptPayment
                                };
                                ...
                                printReceipt('#centerReceiptPreview');
                        ...
                        function loadCenterSettingsAsync() {
                            return $.ajax({
                                type: 'get',
                                url: '{{ url('center-settings') }}'
                            }).done(function(res) {
                        ...
                        function loadReceiptClientDataAsync(clientId) {
                            const activeClientId = clientId || currentClientId || '{{ $GETID }}';
                        ...
                        $(document).on('click.viewPage', '.receipt-email', async function() {
                            const $m = $('#receiptPromptModalView');
                            $m.modal('hide');

                            // helper to avoid "undefined" string
                            ...
                            // Prefer .data() if present, otherwise attr()
                            const payment_type = clean($m.data('payment_type') ?? $m.attr('data-payment_type'),
                                clean(latestPaymentData.payment_type, ''));
                            const payment_date = clean($m.data('payment_date') ?? $m.attr('data-payment_date'),
                                clean(latestPaymentData.payment_date, ''));
                            const kumon_month = clean($m.data('kumon_month') ?? $m.attr('data-kumon_month'), clean(
                                latestPaymentData.kumon_month, ''));
                            const amount = clean($m.data('amount') ?? $m.attr('data-amount'), clean(
                                latestPaymentData.amount, ''));
                            const reference_no = clean($m.data('reference_no') ?? $m.attr('data-reference_no'),
                                clean(latestPaymentData.reference_no, ''));

                            const email = clean($('.primary-email').text(), '');
                            const clientName = clean($('.client-name').text(), '');

                            const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                            if (!email) {
                                showError('Please enter a primary email address.');
                                return;
                            }
    ...
    `,
                                ...
                                showError('Server error sending payment receipt.');
                            }
                        });
                        ...
                        function populateReceiptPreview() {
                            ...
                            const clientName =
                                ...
                                .trim();
                            const clientAddress = [
                                currentClientData.client_address || currentClientData.address || '',
                                currentClientData.city || '',
                                currentClientData.province || currentClientData.state || '',
                                currentClientData.postal_code || currentClientData.zip || ''
                            ].filter(Boolean).join(', ');

                            const centerAddress = [
                                centerSettingsData.address || '',
                                centerSettingsData.city || '',
                                centerSettingsData.province || '',
                                centerSettingsData.postal_code || ''
                            ].filter(Boolean).join(', ');
                            const formatAddressLines = function(address) {
                                ...
                                const line2 = parts[2] || '';
                                const line3 = parts.slice(3).join(', ');
                                return {
                                    line1,
                                    line2,
                                    line3
                            ...
                            const clientLines = formatAddressLines(clientAddress);
                            const centerLines = formatAddressLines(centerAddress);

                            $('#receiptClientName').text(clientName || 'Client');
                            $('#receiptPaymentMethod').text(latestPaymentData.payment_type || '-');
                            $('#receiptPaymentDate').text(latestPaymentData.payment_date || '-');
                            $('#receiptPaymentMonth').text(latestPaymentData.kumon_month || '-');
                            const amountValue = latestPaymentData.amount !== undefined && latestPaymentData.amount !== null ?
                                `$${parseFloat(latestPaymentData.amount).toFixed(2)}` :
                                '-';
                            $('#receiptPaymentAmount').text(amountValue);
                            $('#receiptPaymentReference').text(latestPaymentData.reference_no || '-');

                            $('#centerReceiptName').text(centerSettingsData.center_name || '');
                            $('#centerReceiptAddress').html(
                                [centerLines.line1, centerLines.line2, centerLines.line3].filter(Boolean).join('<br>')
                            );
                            $('#centerReceiptCity').text('');
                            $('#centerReceiptTelephone').text(centerSettingsData.telephone || '');
                            $('#centerReceiptClientName').text(clientName || 'Client');
                            $('#centerReceiptClientAddress').html(
                                [clientLines.line1, clientLines.line2, clientLines.line3].filter(Boolean).join('<br>')
                            );
                            $('#centerReceiptPaymentMethod').text(latestPaymentData.payment_type || '-');
                            $('#centerReceiptPaymentDate').text(latestPaymentData.payment_date || '-');
                            $('#centerReceiptPaymentMonth').text(latestPaymentData.kumon_month || '-');
                            $('#centerReceiptPaymentAmount').text(amountValue);
                            $('#centerReceiptPaymentReference').text(latestPaymentData.reference_no || '-');
                        }

                        function printReceipt(selector) {
                            const printContents = $(selector).html();
                            const win = window.open('', '_blank', 'width=900,height=700');
                            win.document.write('<html><head><title>Receipt</title>');
                            win.document.write('<style>' +
                                'body{margin:0;padding:0;font-family:Arial,sans-serif;color:#2b2b2b;}' +
                                '.receipt-page{padding:52px 70px;}' +
                                '.receipt-header{display:flex;align-items:flex-start;justify-content:space-between;}' +
                                '.receipt-logo{height:36px;}' +
                                '.receipt-title{font-size:20px;letter-spacing:6px;font-weight:700;margin-top:4px;}' +
                                '.receipt-right{font-size:10px;line-height:1.4;text-align:right;}' +
                                '.receipt-section{margin-top:22px;font-size:10px;}' +
                                '.receipt-label{font-weight:700;width:100px;white-space:nowrap;}' +
                                '.receipt-row{display:flex;margin:8px 0;}' +
                                '.receipt-divider{height:1px;background:#d9d9d9;margin:14px 0;}' +
                                '.receipt-client{margin-top:18px;font-size:10px;line-height:1.6;}' +
                                '.receipt-table{width:100%;font-size:10px;border-collapse:collapse;margin-top:12px;}' +
                                '.receipt-table td{padding:6px 0;vertical-align:top;}' +
                                '</style>');
                            win.document.write('</head><body>');
                            win.document.write(printContents);
                            win.document.write(
                                // '<script>window.onload=function(){setTimeout(function(){},300);};<\/script>'
                        ...
                        $('#printReceiptBtn').on('click', function() {
                            $('#printReceiptModal').one('hidden.bs.modal', function() {
                                printReceipt('#printReceiptPreview');
                            }).modal('hide');
                        });
                        // $('#printCenterReceiptBtn').on('click', function() {
                        //     $('#centerReceiptModal').one('hidden.bs.modal', function() {
                        //         printReceipt('#centerReceiptPreview');
                        //     }).modal('hide');
                        // });

                        $('#centerSettingsBtn').on('click', function() {
                            $.ajax({
                                type: 'get',
                                url: '{{ url('center-settings') }}',
                                success: function(res) {
                                    centerSettingsData = res || {};
                                    $('#administrator_email').val(res.administrator_email || '');
                                    $('#center_name').val(res.center_name || '');
                                    $('#center_address').val(res.address || '');
                                    $('#center_city').val(res.city || '');
                                    $('#center_province').val(res.province || '');
                                    $('#center_postal_code').val(res.postal_code || '');
                                    $('#center_telephone').val(res.telephone || '');
                        ...
                        $('#saveCenterSettings').on('click', function() {
                            const payload = {
                                administrator_email: $('#administrator_email').val().trim(),
                                center_name: $('#center_name').val().trim(),
                                address: $('#center_address').val().trim(),
                                city: $('#center_city').val().trim(),
                                province: $('#center_province').val().trim(),
                                postal_code: $('#center_postal_code').val().trim(),
                                telephone: $('#center_telephone').val().trim()
                            };

                            validateFieldByIdOrName('administrator_email', 0, {
                                type: 'email'
                            });
                            validateFieldByIdOrName('center_name', 0);
                            validateFieldByIdOrName('center_address', 0);
                            validateFieldByIdOrName('center_city', 0);
                            validateFieldByIdOrName('center_province', 0);
                            validateFieldByIdOrName('center_postal_code', 0);
                            validateFieldByIdOrName('center_telephone', 0);

                            // show toast if any empty
                            if (Object.values(payload).some(v => !v)) {
                                $('#centerSettingsModal .form-validation-toast').fadeIn();
                                setTimeout(() => $('#centerSettingsModal .form-validation-toast').fadeOut(), 3000);
                                return;
                            }

                            // ✅ if email invalid, stop (validator will style field)
                            if (!validateFieldByIdOrName('administrator_email', 0, {
                                    type: 'email'
                                })) {
                                $('#centerSettingsModal .form-validation-toast').fadeIn();
                                setTimeout(() => $('#centerSettingsModal .form-validation-toast').fadeOut(), 3000);
                            ...
                            $.ajax({
                                ...
                                success: function(res) {
                                    if (res && res.success) {
                                        centerSettingsData = payload;
                                        $('#centerSettingsModal').modal('hide');
                                        if (typeof showSuccessNotify === 'function') {
                                            showSuccessNotify('Saved', 'Center settings updated.');
                                        }
                                    } else {
                                        showError(res.message || 'Unable to save settings.');
                                    }
                                },
                                error: function() {
                        ...
                        function validateFieldByIdOrName(selector, showMessage = 1, rules = {}) {
                            let field;
                            ...
                            if (borderDiv.length === 0) borderDiv = field.prev('.edit-border');

                            const title = borderDiv.find('h6');
                            const icon = borderDiv.find('.constant-icon');

                            // remove old messages
                            field.closest('.form-group, .input-wrapper').find('.validation-error').remove();
                            if (borderDiv.length) borderDiv.siblings('.validation-error').remove();
                            else field.siblings('.validation-error').remove();

                            // helper to apply error UI
                            const applyErrorUI = (msg) => {
                                if (borderDiv.length) {
                                ...
                                if (showMessage == 1) {
                                    const errorTarget = borderDiv.length ? borderDiv : field;
                                    if (!errorTarget.next('.validation-error').length) {
                                        errorTarget.after(
                                            `<div class="validation-error">
                        <i class="fa-light fa-triangle-exclamation text-orange fs-16 mr-2"></i> ${msg}
                     </div>`
                                        );
                                    }
                                }
                            ...
                            // ✅ required check
                            if (!fieldVal) {
                                applyErrorUI('Field validation failed.');
                                return false;
                            }
                            ...
                            if (rules.type === 'email') {
                                ...
                                if (!emailPattern.test(fieldVal)) {
                                    applyErrorUI('Please enter a valid email address.');
                                    return false;
                                }
                            }
                        ...
                        $('#openExportModal').on('mouseenter', function() {
                            $(this).find('i').addClass('fa-shake');
                        });

                        $('#openExportModal').on('mouseleave', function() {
                            $(this).find('i').removeClass('fa-shake');
                        });

                        $(document).on('mouseenter', '.client-read-phone-btn', function() {
                            $(this).find('i').addClass('fa-shake');
                        });

                        $(document).on('mouseleave', '.client-read-phone-btn', function() {
                            $(this).find('i').removeClass('fa-shake');
                        });

                        if (typeof window.showToast !== 'function') {
                            window.showToast = function(cls, timeout = 3000) {
                                const $el = $('.' + cls);
                                $el.stop(true, true);
                        ...
                        $('#confirmExport').on('click', function() {
                            ...
                            if (!type) {
                                showError('Please select an export type.');
                                return;
                            }
                            $('#exportModal').modal('hide');
                            $('#exportProgressModal').modal('show');
                            //showToast('client-export-toast');
                            const exportUrl = "{{ url('/clients/export') }}?type=" + encodeURIComponent(type);
                            setTimeout(() => {
                                fetch(exportUrl, {
                                    ...
                                    .then(({
                                        ...
                                        Dashmix.helpers('notify', {
                                            ...
                                            message: `
    <div>
      <div class="font-titillium text-renue-blue" style="font-weight: 800; font-size: 15pt;">Export Completed</div>
      <div class="d-flex align-items-center">
          <div style="font-size: 14pt; margin-right: 8px;"><i class="fa-thin fa-arrow-up-from-bracket text-darkgrey"></i></div>
          <div>Export successfully completed.</div>
      </div>
    </div>
    `,
                                            allow_dismiss: true,
                            ...
                            $.ajax({
                                ...
                                success: function(res) {
                                    if (res && res.success) {
                                        ...
                                        // showToast('client-import-toast');
                                        sessionStorage.setItem('successTitle', "Import successfull");
                                        sessionStorage.setItem('successMessage',
                                            "Import completed successfully.");

                                        location.reload();
                                    } else {
                                        const message = (res && typeof res.message === 'string' && res
                                                .message.trim()) ?
                                            res.message :
                                            'Import failed.';
                                        showError(message);
                                ...
                                error: function(xhr) {
                                    const message = xhr.responseJSON && typeof xhr.responseJSON.message ===
                    ...
                    $(document).on('click', '.read-nav-tabs .nav-link', function() {
                        ...
                        localStorage.setItem('client_active_tab', currentTab);
                        localStorage.setItem('client_active_tab_target', target);
                    })
            ...
            <script type="text/javascript">
                $(function() {
                    ...
                    $(document).on('change', 'input[name="email_notification_type"]', function() {
                        $('label[for="email_notification_type_payment"]').removeClass('active');
                        $('label[for="email_notification_type_vacation"]').removeClass('active');
                    ...
                    $(document).on('click', '.btn-notify-email', function() {
                        const closeNotifyModal = function() {
                            const $modal = $('#notifyEmailsModal');
                            $modal.modal('hide');
                        ...
                        let client_id = $(this).data('client-id');
                        let notification_type = $('input[name="email_notification_type"]:checked').val() ||
                            'payment';
                    ...
                    $(document)
                        .on('mouseenter', '.dropdown-toggle', function() {
                            $(this).find('i')
                                .removeClass('fa-thin fa-ellipsis-stroke-vertical')
                                .addClass('fa-solid fa-ellipsis-vertical');
                        })
                        .on('mouseleave', '.dropdown-toggle', function() {
                            $(this).find('i')
                                .removeClass('fa-solid fa-ellipsis-vertical')
                                .addClass('fa-thin fa-ellipsis-stroke-vertical');
                        });
                    ...
                    $(document).on('click', '#downloadStudentCSV', function() {
                        ...
                        // Headers
                        let rows = [
                            ["Student Name", "Subject 1", "Subject 2", "Subject 3", "Start Date", "Amount"]
                        ];

                        table.find("tbody tr").each(function() {
                            let student_name = $(this).find('.student-name').text().trim(),
                                subjects = $(this).find('td.subjects .subject-text').text().trim().split(
                                    ",").map(s => s.trim()),
                                start_date = $(this).find('.start-date').text().trim(),
                                amount = $(this).find('.amount').text().trim().replace(/,/g, '');

                            // Limit to 3 subjects
                            let subCols = [];
                            for (let i = 0; i < 3; i++) {
                                subCols.push(subjects[i] || ''); // fill empty if less than 3
                            }

                            rows.push([student_name, ...subCols, start_date, amount]);
                        });

                        // Wrap fields in quotes for CSV
                        ...
                        a.download = "student-details-" + timestamp + ".csv";
                        a.click();

                        showToast('student-download-toast');
                    });


                    $(document).on('mouseenter', '.address-container', function() {
                        $(this).find('.copy-address-text').fadeIn();
                    }).on('mouseleave', '.address-container', function() {
                        $(this).find('.copy-address-text').fadeOut();
                    });
                    $(document).on('mouseenter', '.copy-address-text', function() {
                        $(this).find('i').removeClass('fa-thin').addClass('fa-solid');
                    }).on('mouseleave', '.copy-address-text', function() {
                        $(this).find('i').removeClass('fa-solid').addClass('fa-thin');
                    });

                    $(document).on('click', '.copy-address-text', function() {
                        const container = $(this).closest('.address-container');

                        let textToCopy = container.find('.address-copy-content').text();
                        ...
                        navigator.clipboard.writeText(textToCopy).then(() => {
                            const toast = container.find('.address-copy-toast');
                            toast.fadeIn(200);
                    ...
                    $(document).on('click', '#downloadPaymentCSV', function() {
                        var table = $('.payment-table');

                        let rows = [
                            ["Month", "Payment Type", "Reference No.", "Amount", "Date"]
                        ];

                        table.find("tbody tr").each(function() {
                            let month = $(this).find('.month').text().trim(),
                                payment = $(this).find('.payment-type').text().trim(),
                                reference = $(this).find('.reference-no').text().trim(),
                                amount = $(this).find('.amount').text().trim().replace(/[$,]/g,
                                    ''), // remove $ and commas
                                date = $(this).find('.payment-date').text().trim();

                            rows.push([month, payment, reference, amount, date]);
                        });
                        ...
                        let timestamp = new Date().toISOString().replace(/[:.-]/g, "");
                        a.download = "Payments-details-" + timestamp + ".csv";
                        a.click();

                        showToast('payment-download-toast');
                    });
                    $(document).on('change', 'input[name="vacationToogle"]', function() {
                        var value = $(this).val();

                        if (value === 'active') {
                            // Show active + upcoming rows
                            $('.vacation-table tbody tr').hide();
                            $('.vacation-table tbody tr[data-status="active"], .vacation-table tbody tr[data-status="upcoming"], .vacation-table tbody tr.active, .vacation-table tbody tr.upcoming')
                                .show();
                        } else if (value === 'all') {
                            // Show all rows
                            $('.vacation-table tbody tr').show();
                        }
                    });

                    $(document).on('click', '#downloadVacationCSV', function() {

                        let rows = [
                            ["Student", "Date Range", "Takework", "Comments"]
                        ];

                        $('.vacation-table tbody tr:visible').each(function() {

                            let student = $(this).find('.student_name').text().trim();
                            let dateRange = $(this).find('.date-range').text().trim();
                            let takework = $(this).find('.takework').val() || '';
                        ...
                        // CSV-safe formatting (handles commas, quotes, new lines)
                        let csv = rows.map(row =>
                        ...
                        let a = document.createElement("a");
                        a.href = url;
                        a.download = "vacation-report-" +
                            new Date().toISOString().replace(/[:.-]/g, "") + ".csv";
                        a.click();

                        showToast('vacation-download-toast');
                    });
                    ...
                    $(document).on('click', '.btn-edit, .btn-edit-3dot', function(e) {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                $('#showEditData').removeClass('d-none').addClass('d-flex flex-column')
                                    .html(response);
                                ...
                                $('#nav-main-tab-client-edit').addClass('active');
                                $('#nav-main-client-edit').addClass('show active');
                            },
                            error: function() {
                    ...
                    $(document).on('click', '.btn-clone-confirm', function(e) {
                        e.stopPropagation();
                        var id = $(this).attr('data');
                        var url = "{{ url('clone-client') }}?id=" + id;
                        $(this).closest('.dropdown-menu')?.removeClass('show');
                        $.ajax({
                            url: url,
                            type: 'GET',
                            success: function(response) {
                                ...
                                // 🔹 Insert the new HTML first
                                $('#showEditData').removeClass('d-none').removeClass('d-flex')
                                    .removeClass('flex-column').html(response);
                    ...
                    $(document).off('click.clientRead', '.client-read-phone-btn').on('click.clientRead',
                        '.client-read-phone-btn',
                        function(e) {
                            e.preventDefault();
                            const rawPhone = $(this).data('phone') || '';
                            const cleanedPhone = String(rawPhone).replace(/[^0-9+]/g, '');
                            if (!cleanedPhone) {
                    ...
                    $(document).off('click.clientRead', '.client-read-email-btn').on('click.clientRead',
                        '.client-read-email-btn',
                        function(e) {
                            e.preventDefault();
                            ...
                            if (navigator.clipboard && navigator.clipboard.writeText) {
                                navigator.clipboard.writeText(email).then(() => {
                                    showSuccessNotify('Copied', 'Email address copied.');
                                }).catch(() => {
                    ...
                    $('#updateCommentBtn').on('click', function() {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                localStorage.setItem('client_active_tab',
                    ...
                    $('#insertComment').on('submit', function() {
                        localStorage.setItem('client_active_tab_target', 'nav-comments-client');
                        localStorage.setItem('client_active_tab', 'nav-comments-tab-client');
                    });
                    $('#insertAttachment').on('submit', function() {
                        localStorage.setItem('client_active_tab_target', 'nav-attachments-client');
                        localStorage.setItem('client_active_tab', 'nav-attachments-tab-client');
                    });
                    $('#deleteCommentBtn').on('click', function() {
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                localStorage.setItem('client_active_tab_target',
                                    'nav-comments-client');
                                localStorage.setItem('client_active_tab',
                ...
                `,
                    ...
                    $(document).on('click', '#deleteClientBtn', function() {
                        const formData = $('#deleteClientForm').serialize();
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                showDeleteNotify('Deleted Successfully',
                                    'Client has been deleted successfully.', response.id, response
                                    .id, 'client');
                                setTimeout(function() {
                                    location.reload();
                                }, 4000);
                    ...
                    function showData(id) {
                        ...
                        $.ajax({
                            ...
                            success: function(res) {
                                ...
                                $('.header-item-code').text(res.headerText);
                                ...
                                const isActive = Number(res.is_active) === 1;
                                const statusText = isActive ? 'Active' : 'Inactive';
                                const nextText = isActive ? 'In-Active' : 'Active';
                                const endedValue = isActive ? 0 : 1;
                                const statusIcon = isActive ?
                                    '<i class="fa-light fa-power-off regular-icon"></i>' +
                                    '<i class="fa-solid fa-power-off solid-icon" style="padding-right: 2px; padding-left: 2px;"></i>' :
                                    '<i class="fa-light fa-circle-check regular-icon"></i>' +
                                    '<i class="fa-solid fa-circle-check solid-icon" style="padding-right: 2px; padding-left: 2px;"></i>';
                                $('.btn-toggle-status')
                                    .attr('data', res.id)
                                    .attr('data-status', statusText)
                                    .attr('data-ended', endedValue)
                                    .attr('data-original-title', nextText)
                                    .html(statusIcon);
                                $('.btn-pdf').attr('href', res.pdfUrl);
                                $('.icon-html').html(res.iconHtml);
                                currentClientData = res.client || {};
                                latestPaymentData = res.latest_payment || {};
                                currentClientId = res.id || id;
                                $('[data-toggle=tooltip]').tooltip();
                                ...
                                const targetSection = localStorage.getItem('client_active_tab_target');
                                if (savedTab && targetSection) {
                                    ...
                                    $('.sub-nav-tabs .nav-link').removeClass('active');
                                    $('.tab-pane').removeClass('show active');
                                    $('#nav-main-tab-client').addClass('active');
                                    $('#nav-main-client').addClass('show active');
                                }
                                // cardScroll()
                                ...
                                // } else {
                                //     toggleCards(true); // default ON
                                //     localStorage.setItem('cardsToggleState', 'on');
                                // }
                                $('body').append(
        ...
        `);
                                    ...
                                    $(document).on("mouseenter", ".msg-text.text-ellipsis", function() {
                                        const el = $(this);
                                        const node = el[0];
                                        const fullText = el.text().trim();
                                        ...
                                        const tooltip = $(`
    <div class="tooltip-box">
      <span class="font-titillium text-darkgrey"></span>
    </div>
  `);

                                        tooltip.find("span").text(fullText);

                                        const host = el.closest(
                                            ".message-banner, .message-banner-renew, .message-banner-ended, .message-banner-planning-required, .message-banner-planning-not-required"
                                        );

                                        host.css("position", "relative");
                                        host.append(tooltip);

                                        // ✅ left align with pinned message block (icon + text start)
                                        const msgLeft = host.find(".msg-left").first();
                                        const left = msgLeft.length ? msgLeft.position().left :
                                            el.position().left;

                                        // keep your vertical placement
                                        const top = el.position().top + el.outerHeight() + 6;
                                    ...
                                    $(document).on("mouseleave", ".msg-text.text-ellipsis", function() {
                                        if (activeTooltip) {
                                            activeTooltip.remove();
                                            activeTooltip = null;
                                        }
                    ...
                    $(document).on('click', '.viewContent', function() {
                        ...
                        $('#showEditData').addClass('d-none').removeClass('d-flex').removeClass('flex-column');
                ...
                $(document).on('mouseenter', '.payment-table tbody tr', function() {
                    const icon = $(this).find('i');
                    icon.addClass('fa-solid').removeClass('fa-light');
                });

                $(document).on('mouseleave', '.payment-table tbody tr', function() {
                    const icon = $(this).find('i');
                    icon.addClass('fa-light').removeClass('fa-solid');
                });
