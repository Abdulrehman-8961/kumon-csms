...
class OldAssetsController extends Controller
{
    //
    ...
    {
        $qry = DB::table('asset_maintenance')->where('asset_id', $request->id)->get();
        return response()->json($qry);
    }
    public function getIpDnsAssets(Request $request)
    {
        $qry = DB::table('asset_ip_dns')->where('asset_id', $request->id)->get();
        return response()->json($qry);
    }
    ...
    public function DeletePhysicalAssets(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'Physical Asset Deleted Successfully  <a href="javascript:;" class="  btn-notify btnUndo ml-4" data-id=' . $request->id . ' data-position="' . $qry->position . '">Undo</a>');
    }
    public function DeletePhysicalAssetsUndo(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'Physical Asset Restored Successfully');
    }
    ...
    public function InsertAssets(Request $request)
    {
        ...
        $data = array('emails' => $recipients, 'type' => $request->asset_type, 'client_name' => $client->client_display_name, 'hostname' => $request->hostname, 'description' => $request->role, 'subject' => 'New Asset ' . $asset_type->asset_type_description . ' Created', 'asset_type' => $asset_type->asset_type_description, 'contract_id' => $id, 'from_name' => $settings->from_name, 'system_type' => $system_type->domain_name, 'role' => $request->role, 'sn' => $request->sn);
    ...
    public function UpdateAssets(Request $request)
    {
        ...
        $maintenanceArray = $request->maintenanceArray;
        if (isset($request->maintenanceArray)) {
            DB::table('asset_maintenance')->where('asset_id', $request->id)->delete();
            foreach ($maintenanceArray as $a) {
                $a = json_decode($a);
        ...
        $ipDns_array = $request->ipDnsArray;
        if (isset($request->ipDnsArray)) {
            DB::table('asset_ip_dns')->where('asset_id', $request->id)->delete();
            foreach ($ipDns_array as $a) {
                $a = json_decode($a);
                DB::table('asset_ip_dns')->insert([
                    'asset_id' => $id,
        ...
        $html .= '</div></div>
                        ...
                        <div class="block new-block position-relative mt-3" >
                            ...
                            <div class="block-content pb-0 mt-4 " style="padding-left: 50px;padding-right: 50px;">
                                ...
                                <div class="row justify-content- position-relative inner-body-content push" jid >
                        ...
                        <input type="hidden" name="attachment_array" id="attachment_array" >
                                <div class="row">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        $maintenance = DB::table('asset_maintenance')->where('asset_id', $id)->first();
                                        if ($maintenance) {
                                            $detailText = '';
                                            ...
                                            $html.='<div class="js-task block block-rounded mb-0 animated fadeIn" data-task-id="9" data-task-completed="false" data-task-starred="false">
                                        <table class="table table-borderless table-vcenter mb-0">
                                            <tbody>
                                                <tr>
                                                    <td class="text-center pr-0" style="width: 50px;">
                                                         <p class="ml-3 mb-0 mr-3 rounded px-1" style="margin-top: -5px;background-color: #FFCC00;width: 75px;font-size: 16px;font-weight: 600;">
                                                    ...
                                                    <td class="js-task-content  pl-0">
                                                        <h2 class="mb-0 comments-text">
                                                            ...
                                                            '.$detailText.' &nbsp;&nbsp</div><span class="comments-subtext mandatory ml-0" style="display: block; font-size: 13pt !important;">Maintenance Window</span></h2>
                                                    </td>
                                        ...
                                        $html.='<div class="row form-group mt-5">
 <div class="col-sm-4 t er">
<div class="contract_type_button  w-100 mr-4 mb-3">
  <input type="checkbox" class="custom-control-input" id="disaster_recovery1" name="disaster_recovery" disabled="" value="1" ' . ($q->disaster_recovery1 == 1 ? 'checked' : '') . '>
  <label class="btn btn-new w-100  py-1 font-11pt " for="disaster_recovery1">D/R Plan</label>
