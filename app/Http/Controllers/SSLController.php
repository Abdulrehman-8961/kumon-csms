...
class SSLController extends Controller
{
    ...
    {
        $qry = DB::table('ssl_attachments as s')
            ->select('s.*', 'u.user_image')
            ->leftjoin('users as u', function ($join) {
                $join->on('u.id', '=', 's.added_by');
            })
            ->where('s.ssl_id', $request->id)->get();
        return response()->json($qry);
    }

    public function getCommentsSSL(Request $request)
    {
        $qry = DB::table('ssl_comments as s')
            ->select('s.*', 'u.user_image')
            ->leftjoin('users as u', function ($join) {
                $join->on('u.id', '=', 's.added_by');
            })
    ...
    public function saveEmailCert(Request $request)
    {
        $qry = DB::table('ssl_emails')->insert([
            'renewal_email' => $request->email_address,
            'ssl_id' => $request->cert_id
        ]);
    ...
    public function PrintSSLCertificate(Request $request)
    {
        if (!Auth::check()) {
            ...
            if ($qry == '') {
                ...
                $contract = DB::table('ssl_certificate')->where('is_deleted', 0)->where('id', $qry->contract_id)->first();
                if ($contract == '') {
    ...
    $affliates = $request->affliates==''?'':implode(',',$request->affliates);
        ...
        if ($request->id != '') {
            $id = $request->id;
            $qry = DB::select("select a.*,at.asset_icon from assets as a left join asset_type as at on a.asset_type_id=at.asset_type_id where a.is_deleted=0 and ntp=1 and (a.client_id='$client_id' $where)  and  AssetStatus=1  ");
        } else {
    ...
    public function InsertSSLCertificate(Request $request)
    {
        ...
        if ($request->cert_notification == 1) {
            ...
            try {
                $data = array('emails' => $recipients, 'cert_name' => $request->cert_name, 'cert_edate' => $request->cert_edate, 'subject' => 'New SSL Certificate ' . $request->hostname . ' Created', 'hostname' => $request->hostname, 'cert_id' => $id, 'from_name' => $settings->from_name);
                Mail::send('emails.renewal_email_ssl1', ['data' => $data], function ($message) use ($data) {
    ...
    public function DeleteSSLCertificate(Request $request)
    {
        ...
        return redirect()->back()->with('success', 'SSL Certificate Deleted Successfully  <a href="javascript:;" class="  btn-notify btnUndo ml-4 mt-0 mb-0 float-end float-right" data-id="' . $request->id . '">Undo</a>');
    }
    public function DeleteSSLCertificateUndo(Request $request)
    {
        ...
        DB::table('ssl_certificate')->where('id', $id)->update(['is_deleted' => 0, 'deleted_at' => null]);
        return redirect()->back()->with('success', 'SSL Certificate Restored Successfully');
    }
    ...
    {
        $qry = DB::table('ssl_certificate as s')->select('s.*', 'a.hostname', 'c.client_display_name', 'usr.firstname as created_firstname', 'usr.lastname as created_lastname', 'upd.firstname as updated_firstname', 'upd.lastname as updated_lastname', 'c.logo', 'v.vendor_image')->leftjoin('assets as a', 'a.id', '=', 's.cert_hostname')->leftjoin('vendors as v', 'v.id', '=', 's.cert_issuer')->leftjoin('clients as c', 'c.id', '=', 's.client_id')->leftjoin('users as usr', 'usr.id', '=', 's.created_by')->leftjoin('users as upd', 'upd.id', '=', 's.updated_by')->where('s.is_deleted', 0)->where('s.id', $request->id)->first();
    ...
    public function RenewSSLCertificateUpdate(Request $request)
    {
        ...
        if ($request->cert_notification == 1) {
            ...
            try {
                //code...
                $data = array('emails' => $recipients, 'cert_name' => $request->cert_name, 'cert_edate' => $request->cert_edate, 'subject' => 'Renew SSL Certificate ' . $request->hostname . ' Created', 'hostname' => $request->hostname, 'cert_id' => $id, 'from_name' => $settings->from_name);
                Mail::send('emails.renewal_email_ssl1', ['data' => $data], function ($message) use ($data) {
                    $message->to($data['emails']);
                    $message->subject($data['subject']);
                    $message->from('support@consultationamaltitek.com', $data['from_name']);
...
?>
                                             ...
                                             <a href="javascript:;" class="btnEnd"   data="' . $q->id . '" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Revoke SSL" class=" "><img src="public/img/icon-header-white-end-decom.png?cache=1" width="22px"></a>
                                                ...
                                                <img src="public/img/icon-header-white-reactivate.png?cache=1" width="22px">
                                                                ...
                                                                $html .= '
                                                                <a href="'. url("add-ssl-certificate") .'?id='. $q->id .'" data="' . $q->id . '" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Clone"  style="padding:5px 7px">
                                                <img src="' . asset("public/icons/icon-white-clone.png?cache=1") . '" width="22px"  >
                                                                <a  target="_blank" href="' . url('pdf-ssl-certificate') . '?id=' . $q->id . '"  data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="Pdf" class=" " style="padding:5px 7px">
                                        ...
                                        <div class="form-group row">
                                         ...
                                         <div class="col-sm-4">
                                           <div class="bubble-new">Start Date</div> 
                                            ...
                                            <div class="col-sm-3">
                                                  <div class="bubble-white-new bubble-text-sec">
                                           ' . date('Y-M-d', strtotime($q->cert_sdate)) . '
                                                </div> 
                                            </div>
                                               <div class="col-sm-2">
                                           <div class="bubble-new">End</div> 
                                       </div>
                                     <div class="col-sm-3">
                                                  <div class="bubble-white-new bubble-text-sec">
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="js-task-content  pl-0" style="">
                                                        <label class="mb-0">' . $s->san . '</label>
                                                                ...
                                                                $line_items = DB::table('ssl_host as s')
                                                                                ->selectRaw("
                                                                                    *, 
                                                                                    s.id as sid, 
                                                                                    s.hostname as host, 
                                                                                    ...
                                                                                    (
                                                                                        SELECT row_number FROM (
                                                                                            ...
                                                                                            FROM (
                                                                                                SELECT id FROM assets WHERE is_deleted = 0 ORDER BY id ASC
                                                                                            ) l 
                                                                                            JOIN (SELECT @curRow := 0) r
                                                                                        ) t 
                                                                                        WHERE t.id = s.hostname 
                                                                                        LIMIT 1
                                                                                    ) as rownumber
                                                                                ")
                                                                                ->leftJoin('assets as a', 'a.id', '=', 's.hostname')
...
<div class="d-flex block-content align-items-center px-2 py-2"><p class="font-11pt mr-1   mb-0  ' . ($l->asset_type == 'physical' ? 'c4-p' : 'c4-v') . ' " style="max-width:20px; " data="262">' . ($l->asset_type == 'physical' ? 'P' : 'V') . '</p><p class="font-12pt mb-0 text-truncate   c4" data-toggle="tooltip" data-trigger="hover" data-placement="top" data-title="'. $l->sn .'" data-html="true" data-original-title="" style="  background-color: rgb(151, 192, 255); color: rgb(89, 89, 89); border-color: rgb(89, 89, 89);" data="262">' . strtolower($l->fqdn) . '</p>';
                  ...
                  <a class="dropdown-item d-flex align-items-center px-0" target="_blank" href="' . url('assets') . '/' . ($l->asset_type) . '?id=' . $l->id . '&page=' . ceil($l->rownumber / 10) . '">   <div style="width: 32;  padding-left: 2px"><img src="public/img/open-icon-removebg-preview.png?cache=1" width="22px"  > &nbsp;&nbsp;View Asset</div></a>  
                                                                ...
                                                                $contract = DB::table('ssl_comments as v')->select('v.*', 'u.user_image')->leftjoin('users as u', 'u.id', '=', 'v.added_by')->where('ssl_id', $q->id)->get();
                                                ...
                                                <tr>
                                                    <td class="text-center pr-0" style="width: 38px;">
                                                         <h1 class="mb-0 mr-3  text-white rounded px-1" style=""><b>
                                                          <img width="40px" height="40px" style="border-radius: 50%;" class="' . ($c->user_image == '' ? 'bg-dark' : '') . '" src="' . ($c->user_image == '' ? 'public/img/profile-white.png' : 'public/client_logos/' . $c->user_image) . '"> </b></h1>
                                                    </td>
                                                ...
                                                <tr>
                                                    <td colspan="2" class="pt-0">
                                                       <p class="px-4 mb-0 comments-section-text">  ' . nl2br($c->comment) . '
                                                                ...
                                                                $contract = DB::table('ssl_attachments as v')->select('v.*', 'u.user_image')->leftjoin('users as u', 'u.id', '=', 'v.added_by')->where('ssl_id', $q->id)->get();
 ...
 <a href="public/temp_uploads/' . $c->attachment . '" download target="_blank" class="text-truncate   attachmentDivNew comments-section-text"><img src="public/img/' . $icon . '"  width="25px"> &nbsp;<span class="text-truncate  " >' . substr($c->attachment, 0, 25) . '</span>
                                                                ...
                                                                $contract = DB::table('ssl_audit_trail as c')->select('c.*', 'u.firstname', 'u.lastname', 'u.user_image')->leftjoin('users as u', 'u.id', '=', 'c.user_id')->where('c.ssl_id', $q->id)->get();
