...
class HomeController extends Controller
{
    ...
    public function CheckContractStatus(Request $request)
    {
        ...
        $qry = DB::Table('contracts as c')->select('c.*', 'cl.renewal_notification_email', 'cl.renewal_notification', 'v.vendor_name')
            ->leftjoin('clients as cl', 'cl.id', '=', 'c.client_id')->leftjoin('vendors as v', 'v.id', '=', 'c.vendor_id')
            ->where('c.contract_status', 'Expired')->where('c.is_deleted', 0)
            ->get();
        foreach ($qry as $q) {
            // Declare two dates
        ...
        foreach ($qry as $q) {
            ...
            if ($days == -1 * $settings->interval_1) {
                $subject = $settings->interval_1 . " days left to Renew " . $q->contract_description;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_2) {
                $subject = $settings->interval_2 . " days left  to Renew " . $q->contract_description;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_3) {
                $subject = $settings->interval_3 . " days left to Renew " . $q->contract_description;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_4) {
                $subject = $settings->interval_4 . " days left to Renew  " . $q->contract_description;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_5) {
                $subject = $settings->interval_5 . " days left to Renew  " . $q->contract_description;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_6) {
                $subject = $settings->interval_6 . " days left to Renew  " . $q->contract_description;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_7) {
                $subject = $settings->interval_7 . " days left to Renew  " . $q->contract_description;
                $result = 1;
            }
            echo ($days . '<br>');
            echo $result;
            if ($result == 1) {
                if ($q->renewal_notification == 1 && $q->email_sent != date('Y-m-d')) {
                    ...
                    $data = array('emails' => $client_emails, 'contract_no' => $q->contract_no, 'contract_description' => $q->contract_description, 'start_date' => $q->contract_start_date, 'end_date' => $q->contract_end_date, 'vendor_name' => $q->vendor_name, 'contract_id' => $q->id, 'subject' => $subject, 'from_name' => $settings->from_name);
    ...
    public function CheckSSLStatus(Request $request)
    {
        ...
        $qry1 = DB::Table('ssl_certificate as c')->select('c.*', 'a.hostname')
            ->leftjoin('assets as a', 'a.id', '=', 'c.cert_hostname')
            ->where('c.cert_status', 'Active')->where('c.is_deleted', 0)->get();
        foreach ($qry1 as $q) {
            $result = 0;
            $start_date = strtotime($q->cert_edate);
            $end_date = strtotime(date("Y-m-d"));
            $days = ($end_date - $start_date) / 60 / 60 / 24;
            // if($q->id == 1) {
            //     dd($q->cert_edate, $days, $settings);
            // }
            if ($days == -1 * $settings->interval_1) {
                // dd($days);
                $subject = $settings->interval_1 . " days left to Renew of SSL Certificate  " . $q->description;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_2) {
                // dd($days);
                $subject = $settings->interval_2 . " days left to Renew of SSL Certificate  " . $q->description;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_3) {
                // dd($days);
                $subject = $settings->interval_3 . " days left to Renew of SSL Certificate  " . $q->description;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_4) {
                dd($days);
                $subject = $settings->interval_4 . " days left to Renew of SSL Certificate  " . $q->description;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_5) {
                dd($days);
                $subject = $settings->interval_5 . " days left to Renew of SSL Certificate  " . $q->description;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_6) {
                $subject = $settings->interval_6 . " days left to Renew of SSL Certificate  " . $q->description;
                $result = 1;

                if ($days == -1 * $settings->interval_7) {
                    $subject = $settings->interval_7 . " days left to Renew of SSL Certificate  " . $q->description;
                    $result = 1;
                }
                if ($q->cert_notification == 1) {
                    if ($result == 1) {
                        if ($q->cert_notification == 1 && $q->email_sent != date('Y-m-d')) {
                            ...
                            $data = array('emails' => $client_emails, 'cert_name' => $q->cert_name, 'description' => $q->description, 'cert_sdate' => $q->cert_sdate, 'cert_edate' => $q->cert_edate, 'ssl_id' => $q->id, 'subject' => $subject, 'hostname' => $q->hostname, 'from_name' => $settings->from_name);
        ...
        foreach ($qry1 as $q) {
            ...
            if ($days == -1 * $settings->interval_1) {
                $subject = $settings->interval_1 . " days left to Renew of SSL Certificate " . $q->decsription;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_2) {
                $subject = $settings->interval_2 . " days left to Renew of SSL Certificate  " . $q->decsription;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_3) {
                $subject = $settings->interval_3 . " days left to Renew of SSL Certificate " . $q->decsription;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_4) {
                $subject = $settings->interval_4 . " days left to Renew of SSL Certificate " . $q->decsription;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_5) {
                $subject = $settings->interval_5 . " days left to Renew of SSL Certificate " . $q->decsription;
                $result = 1;
            }
            if ($days == -1 * $settings->interval_6) {
                $subject = $settings->interval_6 . " days left to Renew of SSL Certificate " . $q->decsription;
                $result = 1;
            }

            if ($days == -1 * $settings->interval_7) {
                $subject = $settings->interval_7 . " days left to Renew of SSL Certificate " . $q->decsription;
                $result = 1;
            }
    ...
    public function contractTestEmail(Request $request)
    {
        $id = $request->query('id');
        $email = $request->query('email');
        $settings = DB::Table('notification_settings')->first();
        $q = DB::Table('contracts as c')->select('c.*', 'cl.renewal_notification_email', 'cl.renewal_notification', 'v.vendor_name')->leftjoin('clients as cl', 'cl.id', '=', 'c.client_id')->leftjoin('vendors as v', 'v.id', '=', 'c.vendor_id')->where('c.is_deleted', 0)->where('c.id', $id)->first();
        $start_date = strtotime($q->contract_end_date);
        $end_date = strtotime(date("Y-m-d"));
        $days = ($start_date - $end_date) / 60 / 60 / 24;
        $subject = $days . " days left  to renew of " . $q->contract_description;
        $c = DB::Table('client_emails')->where('client_id', $q->client_id)->first();

        // Prepare data array for email template
        $data = [
            'emails' => $c->renewal_email,
            'contract_no' => $q->contract_no,
            'contract_description' => $q->contract_description,
            'start_date' => $q->contract_start_date,
            'end_date' => $q->contract_end_date,
            'vendor_name' => $q->vendor_name,
            'contract_id' => $id,
            'subject' => $subject,
            'from_name' => 'Consultation AmaltiTEK Notification',
        ];
        Mail::send('emails.renewal_email', ['data' => $data], function ($message) use ($data, $email) {
            $message->to($email);
            $message->subject($data['subject']);
            $message->from('support@consultationamaltitek.com', $data['from_name']);
        });
    ...
    public function testEmailSSL(Request $request)
    {
        $id = $request->query('id');
        $email = $request->query('email');
        $settings = DB::Table('notification_settings')->first();
        $q = DB::Table('ssl_certificate as c')->select('c.*', 'a.hostname')->leftjoin('assets as a', 'a.id', '=', 'c.cert_hostname')->where('c.id', $id)->where('c.is_deleted', 0)->first();
        // dd($q);
        $vendor_name = '';
        if ($q->cert_type == 'public') {
            $vendor_data = DB::table('vendors')->where('id', $q->cert_issuer)->first();
            $vendor_name = @$vendor_data->vendor_name;
        } else {

            $vendor_data = DB::table('clients')->where('id', $q->client_id)->first();
        ...
        $end_date = strtotime(date("Y-m-d"));
        $days = ($start_date - $end_date) / 60 / 60 / 24;
        $c = DB::Table('ssl_emails')->where('ssl_id', $q->id)->first();
        $subject = $days . " left to Renew " . $q->description;
        $data = [
            'cert_name' => $q->cert_name,
            'description' => $q->description,
            'cert_sdate' => $q->cert_sdate,
            'cert_edate' => $q->cert_edate,
            'ssl_id' => $id,
            'subject' => $subject,
            'hostname' => $q->hostname,
        ...
        Mail::send('emails.renewal_email_ssl', ['data' => $data], function ($message) use ($data, $email) {
            $message->to($email);
            $message->subject($data['subject']);
            $message->from('support@consultationamaltitek.com', $data['from_name']);
        });
