...
$result=0;
         ...
         if($days==-1*$settings->interval_1){
                $subject="60 days left to renew of ".$q->contract_no;
                $result=1;
        }
        if($days==-1*$settings->interval_2){
                 $subject="45 days left  to renew of ".$q->contract_no;   
            $result=1;
        }        
        if($days==-1*$settings->interval_3){
                $subject="30 days left to renew of ".$q->contract_no;            
            $result=1;
        }
        if($days==-1*$settings->interval_4){
                $subject="15 days left to renew of  ".$q->contract_no;            
            $result=1;
        }
        if($days==-1*$settings->interval_5){
                $subject="7 days left to renew of  ".$q->contract_no;            
            $result=1;
        }
        if($days==-1*$settings->interval_6){
                $subject="1 days left to renew of  ".$q->contract_no;            
            $result=1;
        }
        if($days==-1*$settings->interval_7){
                $subject="0 days left to renew of  ".$q->contract_no;            
            $result=1;
        }
echo ($days.'<br>');
echo $result;
            if($result==1){
                  ...
                  $data = array( 'emails' => $client_emails, 'contract_no' =>$q->contract_no,'contract_description'=>$q->contract_description,'start_date'=>$q->start_date,'end_date'=>$q->contract_end_date,'vendor_name'=>$q->vendor_name,'contract_id'=>$q->id, 'subject' => $subject,'from_name'=>$settings->from_name);
...
$qry1=DB::Table('ssl_certificate as c')  ->select('c.*','a.hostname')->leftjoin('assets as a','a.id','=','c.cert_hostname') ->where('c.cert_status','Active')->where('c.cert_type','public')->where('c.is_deleted',0)->get();
      ...
      $days= ($end_date - $start_date)/60/60/24;
         ...
         if($days==-1*$settings->interval_1){
                $subject="60 days left to renew of ".$q->decsription;
                $result=1;
        }
        if($days==-1*$settings->interval_2){
                 $subject="45 days left to renew of  ".$q->decsription;   
            $result=1;
        }        
        if($days==-1*$settings->interval_3){
                $subject="30 days left to renew of ".$q->decsription;            
            $result=1;
        }
        if($days==-1*$settings->interval_4){
                $subject="15 days left to renew of ".$q->decsription;            
            $result=1;
        }
        if($days==-1*$settings->interval_5){
                $subject="7 days left to renew of ".$q->decsription;            
            $result=1;
        }
         if($days==-1*$settings->interval_6){
                $subject="2 days left to renew of ".$q->decsription;            
            $result=1;
        }
     
        if($days==-1*$settings->interval_7){
                $subject="0 days left to renew of ".$q->decsription;            
            $result=1;
        }
