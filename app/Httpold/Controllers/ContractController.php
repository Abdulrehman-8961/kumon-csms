 ...
 return response()->json('success');
    ...
    public function GetContractNotifications(Request $request){
           $clients=DB::table('clients')->where('id',$request->id)->first();
            ...
            $email_qry[]=['renewal_email'=>$clients->email_address];
...
$html.='  <div class="block new-block position-relative mt-5" >
                                                ...
                                                <tr>
                                                    <td class="text-center pr-0" style="width: 38px;">
                                                         <h1 class="mb-0 mr-3  text-white bg-dark rounded px-1" style=""><b>
                                                          '.$c->qty.'  <!-- <img width="40px" src="'.($c->user_image==''?'public/img/profile-white.png':'public/client_logos/'.$c->user_image).'"> --></b></h1>
                                                    </td>
  ...
  if($c->contract_type_line!=''){
      $html.='<div class="contract_type_button px-1 ml-2" style=" flex-shrink: 0;" data-toggle="tooltip" data-trigger="hover" data-placement="top" title="" data-original-title="">
        <input type="checkbox" class="custom-control-input" id="patched" name="patched" value="1" disabled="" checked="">
        <label class="btn btn-new w-100 py-1" style="font-size: 8pt !important; color: #595959; border-color: #595959;" for="patched">
          '.($c->contract_type_line).'
        </label>
      </div>';
  }
$html.='</div>
