...
$field=$_GET['field'];
     ...
     $qry=DB::table('sites as s')->where('s.is_deleted',0)->select('s.*','c.client_display_name','c.logo')->join('clients as c','c.id','=','s.client_id')->where(function($query){
...
$qry=DB::table('sites as s')->where('s.is_deleted',0)->select('s.*','c.client_display_name','c.logo')->join('clients as c','c.id','=','s.client_id') ->orderBy('s.id','desc')->get(); 
