...
if(Auth::user()->role=='admin'){
     ...
     $qry=DB::table('ssl_certificate as s')->select('s.*','a.hostname','c.client_display_name','i.vendor_name as issuer','i.vendor_image','c.logo','st.site_name','i.vendor_name')->leftjoin('assets as a','a.id','=','s.cert_hostname')->leftjoin('sites as st','s.site_id','=','st.id')->leftjoin('vendors as i','s.cert_issuer','=','i.id')->join('clients as c','c.id','=','s.client_id')->where(function($query) use ($sear){
...
$query->Orwhere('cert_company','like','%'.@$_GET['search'].'%');
   ...
   $query->Orwhere('cert_state','like','%'.@$_GET['search'].'%');
    ...
    if(Auth::user()->role=='admin'){
$qry=DB::table('ssl_certificate as s')->select('s.*','a.hostname','c.client_display_name','i.vendor_name as issuer','i.vendor_image','c.logo')->leftjoin('assets as a','a.id','=','s.cert_hostname')->leftjoin('vendors as i','s.cert_issuer','=','i.id')->join('clients as c','c.id','=','s.client_id')->where('s.is_deleted',0)->orderBy('s.id','desc')->get(); 
}
else{
    $qry=DB::table('ssl_certificate as s')->select('s.*','a.hostname','c.client_display_name','i.vendor_name as issuer','i.vendor_image','c.logo')->leftjoin('assets as a','a.id','=','s.cert_hostname')->leftjoin('vendors as i','s.cert_issuer','=','i.id')->join('clients as c','c.id','=','s.client_id')->whereIn('s.client_id',$userAccess)->where('s.is_deleted',0)->orderBy('s.id','desc')->get(); 
}
