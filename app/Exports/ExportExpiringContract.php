...
$qry=DB::table('contracts as a')->select('a.*','c.client_display_name','s.site_name','d.distributor_name','v.vendor_name') ->join('clients as c','c.id','=','a.client_id')->join('sites as s','s.id','=','a.site_id')->leftjoin('distributors as d','d.id','=','a.distributor_id')->leftjoin('vendors as v','v.id','=','a.vendor_id')->whereRaw("c.is_deleted=0 $cond")->where('s.is_deleted',0)->where('c.is_deleted',0)->where('a.is_deleted',0)->where('a.contract_end_date','<=',$expiry_date)->where('a.contract_status','Active')->orderBy($field,$orderby) ->get(); 
...
$qry=DB::table('contracts as a')->where(function($query){
  ...
  $query->Orwhere('contract_end_date','like','%'.@$_GET['search'].'%');
     ...
     }) ->select('a.*','c.client_display_name','s.site_name','d.distributor_name','v.vendor_name') ->join('clients as c','c.id','=','a.client_id')->join('sites as s','s.id','=','a.site_id')->leftjoin('distributors as d','d.id','=','a.distributor_id')->leftjoin('vendors as v','v.id','=','a.vendor_id')->where('s.is_deleted',0)->where('c.is_deleted',0)->where('a.is_deleted',0)->where('a.contract_end_date','>=',date('Y-m-d'))->where('a.contract_end_date','<=',$expiry_date)->where('a.contract_status','Active')->orderBy($field,$orderby) ->get(); 
}
else{
...
if(Auth::user()->role=='admin'){
$qry=DB::table('contracts as a')->select('a.*','c.client_display_name','s.site_name','d.distributor_name','v.vendor_name') ->join('clients as c','c.id','=','a.client_id')->join('sites as s','s.id','=','a.site_id')->leftjoin('distributors as d','d.id','=','a.distributor_id')->leftjoin('vendors as v','v.id','=','a.vendor_id')->where('s.is_deleted',0)->where('c.is_deleted',0)->where('a.is_deleted',0)->where('a.contract_end_date','>=',date('Y-m-d'))->where('a.contract_end_date','<=',$expiry_date)->orderBy('a.id','desc') ->get(); 
 }
 else{
