...
if(isset($_GET['advance_search'])){
 ...
 if(isset($_GET['vlan_id'])  && $_GET['vlan_id']!=''){
  ...
  if(isset($_GET['sign_in_method'])  && $_GET['sign_in_method']!=''){
     ...
     $qry=DB::table('network as n')->select('n.*','c.client_display_name','s.site_name','c.logo')->whereRaw("n.is_deleted=0 $cond")->where(function($query){
...
$qry=DB::table('network as n')->select('n.*','c.client_display_name','s.site_name','c.logo')->where('s.is_deleted',0)->where('c.is_deleted',0)->where('n.is_deleted',0)->join('clients as c','c.id','=','n.client_id')->join('sites as s','s.id','=','n.site_id') ->orderBy('n.id','desc')->get(); 
